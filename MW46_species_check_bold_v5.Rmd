...
metabar_unmatched <- metabar %>%
  select(asv_code, asv_seq, phylum, order, family, genus, species, match) %>% filter(!asv_code %in% old_mbc_matched$asv_code) %>% distinct()
...
metabar_matched <- metabar %>%
  select(asv_code, asv_seq, phylum, order, family, genus, species, match) %>%
...
# write file to check agains databases
write.csv(all_mismatched_ids, paste0(wd, "/synonym_updates/MW46_all_mismatched_ids.csv"), row.names = FALSE)
...
taxonomy_cols <- c("asv_seq", "phylum", "order", "family", "genus", "species", "match")
...
write.csv(metabar_unmatched, paste0(wd, "/synonym_updates/MW46_unmatched_for_sequences.csv"), row.names = FALSE)
```
...
CHRIS2: I haven't got passed this point I decided to check all unmatched and all matched data for the 46 site an have started the process. I need another day to finish

```{r}
wd <- "~/uomShare/wergStaff/MelCarew/git-data/Spring_2018_DNA_metabarcoding_data/synonym_updates/BOLDv5_search_results"

# read in .csv files with bold barcodeID engine results.
bold_results1 <- read.csv(paste0(wd, "/MW46_1_500_BOLDsearch_results.csv"))
bold_results2 <- read.csv(paste0(wd, "/MW46_501_1000_BOLDsearch_results.csv"))
bold_results3 <- read.csv(paste0(wd, "/MW46_1001_1500_BOLDsearch_results.csv"))
bold_results4 <- read.csv(paste0(wd, "/MW46_1501_1779_BOLDsearch_results.csv"))
bold_results5 <- read.csv(paste0(wd, "/MW46_BOLDsearch_NOMATCH_results.csv"))

# combine all results into a single file
bold_results_all <- rbind(bold_results1, bold_results2, bold_results3, bold_results4,  bold_results5)
rm(bold_results1, bold_results2, bold_results3, bold_results4,  bold_results5)
...
# extract duplicate asvs in metabar_unmatched
mismatched_ids3 <- metabar_unmatched[metabar_unmatched$asv_code %in% asv_dup$asv_code, ]
...
metabar_unmatched_ck <- metabar_unmatchedp %>%
filter(
...
write.csv(metabar_unmatched,paste0(wd, "/synonym_updates/metabar_unmatched_for_checking.csv"))
...
metabar_unmatched_checked <- read.csv(paste0(wd, "/synonym_updates/MW46_metabar_mismatchs_checked.csv"))
```
...
length(unique(MW46_metabar_asvs_checked$asv_code))  
nrow(unique(MW46_metabar_asvs_checked[c("asv_code","asv_seq")])) #all 407, so no dodgy duplicates -good
## All looks good to me!
```

Try in sequences with less that 97% match
```{r}
# pulling out species data without filtering
metabar_b97 <- spring2018_metabarcoding_Nov2023 %>%
  filter(match < 97) %>%
  select(asv_code, asv_seq, species, match) %>%
  distinct()
...
# reduce df so that only one asv_code (asv_code) is present 
metabar_nomatch_unique <- metabar_nomatch %>%
  distinct(asv_code, .keep_all = TRUE)


write.csv(metabar_nomatch_unique,"~/uomShare/wergStaff/MelCarew/git-data/Spring_2018_DNA_metabarcoding_data/synonym_updates/MW46_metabar_nomatchb_unique.csv", row.names = FALSE)
...
```{r}
# read in .csv files with bold barcodeID engine results
bold_results_no_match <- read.csv("~/uomShare/wergStaff/MelCarew/git-data/Spring_2018_DNA_metabarcoding_data/synonym_updates/BOLDv5_search_results/MW46_BOLDsearch_NOMATCH_results.csv")

# rename all results to match other dataframes
bold_results_no_match <- bold_results_no_match %>%
  rename(asv_code = Query.ID) %>%
  rename(bold_species = Species)
...
# summarise so there is one asv_code per row
bold_results_no_match_sum <- bold_results_no_match %>%
  group_by(`asv_code`) %>%
  slice_max(order_by = `ID.`, n = 1) %>%
  ungroup() %>%
  distinct(asv_code, .keep_all = TRUE) # Add this line to remove fully duplicate rows
...
# check bold IDs against synonym IDs and and previous identifications (orig_species)
# Perform a left join to join bold ids to other ids
metabar_nomatch <- metabar_nomatch_unique %>%
  left_join(bold_results_no_match_sum %>% select(asv_code, species, ID., bin_uri), by = "asv_code") 

# update column names to avoid confusion
metabar_nomatch <- metabar_nomatch %>%
  rename(species_orig = species.x, # original names
         species_bold = species.y, # names from bold v5 check
         species_syn = species_new, # names from synonym df
         bold_match = ID.,# % match from bold v5 check
 ...
 write.csv(metabar_nomatch, "~/uomShare/wergStaff/MelCarew/git-data/Spring_2018_DNA_metabarcoding_data/synonym_updates/MW46_metabar_nomatch_for_checking.csv")
```

Final comparison file
```{r}
MW46_metabar_asvs_checked <- read.csv("~/uomShare/wergStaff/MelCarew/git-data/Spring_2018_DNA_metabarcoding_data/synonym_updates/MW46_metabar_nomatch_checked.csv")