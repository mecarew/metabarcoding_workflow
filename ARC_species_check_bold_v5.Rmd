...
```{r}
#CHIRIS#4: hope I am not being to basic here but I want to walk you through this step, so you know exactly what I did and can then do it yourself. Ultimately, I am hoping  we can look at running it from R rather than loading sequences and retrieving data via the web interface, I am think of Rebekah's e-mail in particular the last sentence: "There is currently no integration of the BOLD v5 ID Engine into the BOLD v5 API but you are welcome to input your sequences directly into the ID Engine for identifications of your sequences here:https://id.boldsystems.org/ .Please note that the ID Engine has a throttling threshold and restricts users to 3 submissions per minute, and 40 general web requests per minute in case you decide to automate your querying via the ID Engine." I have no idea how to do this wondered if you might.

# Now to step you through what I did: 
...
#CHRIS#4: All asv's were searched and these were the one that returned a match on bold version 5
...
arc_unmatched_ck  <- arc_unmatched_ck  %>%
  dplyr::rename(species_orig = species.x, # original names
...
CHRIS#3: Also would you like me to change my columns to 'checked_name_species', action and comments? I think I should. Can do this for the tfill and maxp too. CHRIS#4: I have added in code below so you can understand my logic in what I think you should do with the records and what whether they should be added to the tfill(just taxonomy as the match >97) or max-p-update (the match and taxonomy needs updating) for each asv
```{r}
# read in checked file
arc_unmatched_checked <- read.csv(paste0(wd, "/arc_unmatched_checked.csv"))# original file

# not super helpful as it misses some updates (look at the df)
arc_unmatched_checked <- arc_unmatched_checked %>%
  mutate(
    action = case_when(
      bold_match >= 97 & max_p_identity < 97 ~ "*update max-p-identity and all taxonomy",
      bold_match >= 97 & max_p_identity > 97 & species_orig != species_bold ~ "*update taxonomy",
      TRUE ~ "" # Default to empty string if none of the conditions are met
    )
  )
...
arc_unmatched_checked <- arc_unmatched_checked %>%
  mutate(
    database = case_when(
      bold_match >= 97 & `max_p_identity` >= 97 ~ "add to tfill", # records already at species level from vsearch results in miseq 13
      `max_p_identity` < 97 & bold_match >= 97 ~ "add to max-p- update", # new species in bold version 5 so max_p_identity needs updating
      TRUE ~ ""
    )
  )
# note there remaining records had a bold_match or vsearch match of <97 (some at genus level (=>95 but <97 or family level (=>92 but <95)) and or had no match (or below 92 where the certainty of correct assignment gets less certain. None of these are in the asv library but are capture in the final miseq data file for each run.
...
```
CHRIS#4 (please note this was complete in the same way as in chunk 4)
...
#CHRIS#4: this file is updated from my previous version to include all the bold matches against the max_p_update (there are more additional changes in this one missed because I filtered)
# checking all bold matches (94-100)
write.csv(arc_maxp, paste0(wd, "/arc_maxp_for_checking_all.csv"), row.names = FALSE, na = "")
```

CHRIS: We have lots of bins to add to our database, a small number of corrections and some species that have naming updates needed (below: arc_maxp_checked)
...
arc_maxp_checked <- read.csv(paste0(wd, "/arc_maxp_checked_all.csv"))
...
#any(duplicated(arc_maxp_checked$asv_code)) #no new duplicates

#CHRIS#4: Also, an we please delete all names ending in (Unident.) in the max_p_update df in the asv library revision. It is not really something that should be done with DNA data.
```
...
#CHRIS#4: your 3055extra search results
bold_results_ext1 <- read.csv(paste0(wd5, "BOLD_maxp_tfill_extras_1_1000_results.csv"))
bold_results_ext2 <- read.csv(paste0(wd5, "BOLD_maxp_tfill_extras_1001_2000_results.csv"))
bold_results_ext3 <- read.csv(paste0(wd5, "BOLD_maxp_tfill_extras_2001_3000_results.csv"))
bold_results_ext4 <- read.csv(paste0(wd5, "BOLD_maxp_tfill_extras_3000_end_results.csv"))


# combine all results into a single file (add extras here)
bold_results_tfill_all <- rbind(bold_results_tfill1, bold_results_tfill2, bold_results_tfill3, bold_results_tfill4, bold_results_tfill5, bold_results_tfill6, bold_results_tfill7, bold_results_tfill8, bold_results_tfill9, bold_results_tfill10, bold_results_tfill11, bold_results_tfill12, bold_results_tfill13, bold_results_tfill14, bold_results_tfill15,bold_results_tfill16,bold_results_tfill17, bold_results_tfill18, bold_results_tfill19, bold_results_tfill20, bold_results_tfill21, bold_results_tfill22, bold_results_ext1, bold_results_ext2, bold_results_ext3, bold_results_ext4)
...
### CW compilation of bold_results (tfill + maxp) to be separated later.
### This takes few minutes to run
### Mel, I spent too long making this next step work, and I haven't used the result to 
### inform or check any later decisions, but perhaps you will find it useful to check 
### your manual corrections.  There are quite a few bin_uri groups that this script hasn't
### grouped correctly/consistently, but most of them seem to be non-aqu non-macro species
### that I don't care about much.  Nevertheless, I recommend you run the following code
### (It takes a few minutes), and then have a look at how the results compare to your 
### Checked data (below - the object x created at line 545.)
### I'm thinking this is a rabbit hole I shouldn't have gone down.
tfill_maxp <- rbind(tfill,maxp)
bold_ra <- as.data.frame(rbind(bold_results_tfill_all, bold_results_maxp_all))
bold_ra_asvs <- unique(bold_ra$asv_code)
bold_ra_asv_bins <- data.frame(asv_code = NA, species = NA, bin_uri = NA, comment = NA)[0,]
non_matched_asvs <- vector()
matched_no_fam_id <- vector()
for(i in 2909:length(bold_ra_asvs)){
  skipped <- 0
  comment <- NA
  bi <- bold_ra[bold_ra$asv_code == bold_ra_asvs[i] & bold_ra$ID. >= 97,]
  if(nrow(bi) == 0){
    non_matched_asvs <- c(non_matched_asvs, bold_ra_asvs[i])
    skipped <- 1
  }
  if(nrow(bi) > 0){
   if(sum(!is.na(unique(bi$bin_uri))) == 0){
         non_matched_asvs <- c(non_matched_asvs, bold_ra_asvs[i])
         skipped <- 1
    }else{
     if(length(unique(bi$Order)) == 1){
      if(unique(bi$Order) == ""){
        matched_no_fam_id <- c(matched_no_fam_id, bold_ra_asvs[i])
     skipped <- 1
      }}}}
  if(skipped == 0){
  spname <- unique(bi$species)[!is.na(unique(bi$species))]
  bin_urisi <- unique(bi$bin_uri[!is.na(bi$bin_uri)])
  if(bold_ra_asvs[i] == "6da41df0f8a87c6e316035ba6e3b30c9"){
    bi <- bi[1,]
    spname <- bi$species
    comment <- "100% match to AAW3113 (Austropyrgus angasi) - 97.03% to AAW3122 (Austropyrgus centralia/turbatus). The former selected"
  }
  if(length(spname) > 1) stop("1")
  if(length(spname) == 0) {
    bi <- bi[bi$bold_species == bi$bold_species[bi$ID. == max(bi$ID.)][1],]
    spname <- unique(bi$bold_species)
    comment <- ifelse(spname == "", NA,
                 paste0("bold species name used with sim = ", max(bi$ID.)) )
  }
  if(unique(spname) == ""){
    bi_genus <- unique(bi$Genus)[!is.na(unique(bi$Genus))]
    bi_genus <- bi_genus[bi_genus  != ""]
    if(length(bi_genus) == 0){
        subfam <- unique(bi$Subfamily[!is.na(bi$Subfamily)])
        subfam <- subfam[subfam != ""]
        fam <- unique(bi$Family[!is.na(bi$Family)])
        fam <- fam[fam != ""]
        bi_genus <- ifelse(length(subfam) == 0, ifelse(length(fam) == 0, "", fam), subfam)
        if(bi_genus == ""){
          matched_no_fam_id <- c(matched_no_fam_id, bold_ra_asvs[i])
          next()
        }
      }
    if(length(bi_genus) > 1){
      bi$Genus <- unique(bi_genus)[1]
      comment <- "Bold assigns this to more than one genus: the first was chosen"
      }
    if(length(bin_urisi) == 1){
         spname <- paste0(bi_genus[1], " sp. B-", bin_urisi)
    }
    if(length(bin_urisi) == 2){
         spname <- paste0(bi_genus[1], " sp. B-", bin_urisi[1],"/",bin_urisi[2])
    }
    if(length(bin_urisi) > 2){
         spname <- paste0(bi_genus[1], " sp. B-", bin_urisi[1]," group")
    }
  }
  if(substr(spname,1,3) == " sp") stop("4")
  if(length(unique(bi$bin_uri[!is.na(bi$bin_uri)])) > 0)
  bold_ra_asv_bins <- rbind(bold_ra_asv_bins, 
                            data.frame(asv_code = bold_ra_asvs[i], 
                                       species = spname, 
                                       bin_uri = bin_urisi, 
                                       comment = comment))
  }
}
#CHRIS#4: Extracting extra records
results_all <- rbind(tfill, maxp)
results_omitted <- results_all[!results_all$asv_code %in% bold_ra_asvs,]  

write.csv(results_omitted, paste0(wd, "/results_omitted.cvs"))
          
# note most of these records have already been search an had no match but some were definitely missing (the results can be read in above)
```

Here, we prepare the data from the bold search so it can be added to the tfill dataframe
```{r}
...
```{r}
# CHRIS#4: I have used a revised synonym table I made in the MW46_species_check_bold_v5.Rmd to avoid making new duplicate lines
synonyms_revised <- read.csv(paste0(wd, "/synonyms_revised.csv"))
...
#CHRIS#4: this file is updated from my previous version to include all the bold matches against tfill
# checking all bold matches (94-100)

write.csv(arc_tfill, paste0(wd, "/arc_tfill_for_checking_all.csv"), row.names = FALSE, na = "")
```
...
arc_tfill_checked <- read.csv(paste0(wd, "/arc_tfill_checked_all.csv"))

### Compare to bold_ra_asv_bins species names checked above
arc_tfill_checked$species_cw <- NA
for(i in 1:nrow(arc_tfill_checked)){
  if(sum(bold_ra_asv_bins$asv_code == arc_tfill_checked$asv_code[i]) > 0)
  arc_tfill_checked$species_cw[i] <- unique(bold_ra_asv_bins$species[bold_ra_asv_bins$asv_code == arc_tfill_checked$asv_code[i]])
}
x <- unique(arc_tfill_checked[,c("species_orig","species_cw","updated_species_name")])
...
### Actually, I think that's all good.
...
### having corrected all the species_syn entries, duplicates can be removed by
...
cw_changes <- data.frame(species_orig = NA, species_mc_proposed = NA, species_cw_proposed = NA)[0,]
for(i in 1:nrow(arc_tfill_checked)){
  ...
  if(!arc_tfill_checked$species_orig[i] %in% unique(bins$species)){
    ...
    if(grepl("B-", mc_name) & 
     ...
     if(sum(!is.na(binsi$bin_uri))== 2){
       binsi <- binsi[!is.na(binsi$bin_uri),]
        if(!(grepl(binsi$bin_uri[1], arc_tfill_checked$species_orig[i]) | 
           grepl(binsi$bin_uri[2], arc_tfill_checked$species_orig[i]))){
        arc_tfill_checked$updated_species_name[i] <- 
          paste0(strsplit(arc_tfill_checked$updated_species_name[i]," ")[[1]][1],
                                 " sp. ",binsi$bin_uri[1],"/",binsi$bin_uri[2])
        cw_changes <- rbind(cw_changes, data.frame(species_orig = arc_tfill_checked$species_orig[i], 
                                                   species_mc_proposed = mc_name,
                                                   species_cw_proposed = arc_tfill_checked$updated_species_name[i]))
        }
       }
      if(sum(!is.na(binsi$bin_uri)) > 2){
                arc_tfill_checked$updated_species_name[i] <- 
                  paste0(strsplit(arc_tfill_checked$updated_species_name[i]," ")[[1]][1],
                                 " sp. ",binsi$bin_uri[1]," group")
    ## If name had already been changed in this loop replace it in the cw_changes table
    if(mc_name %in% cw_changes$updated_species_name){
    cw_changes <- rbind(cw_changes, data.frame(species_orig = arc_tfill_checked$species_orig[i],
                                               species_mc_proposed = mc_name,
                                               species_cw_proposed = arc_tfill_checked$updated_species_name[i]))
      }
...
### Check that you approve of these three changes - you should probably check that the pairs of bin_urls are sufficiently similar to be lumped:
cw_changes
#CHRIS#3: I closely check this all tomorrow with fresh eyes. Thanks though it is looking good!

#### Check consistency of new bin_uris with existing bins table
check_ck <- arc_unmatched_checked[arc_unmatched_checked$bold_bin_uri %in% unique(bins$bin_uri),] 
name_changes <- data.frame(name_in_tfill = NA, new_name = NA, bin_url = NA)[0,]
for(i in 1:nrow(check_ck)){
  exist_bin_record <-  bins[!is.na(bins$bin_uri) & 
                            bins$bin_uri == check_ck$bold_bin_uri[i],]
  # Found Dixidae sp. MC-4 and Dixidae sp. MC-2 with same bin_uri in bins...
  if(exist_bin_record$species[1] != check_ck$checked_species_name[i]){
    name_changes <- rbind(name_changes, 
                          data.frame(name_in_tfill = exist_bin_record$species, 
                                     new_name = check_ck$checked_species_name[i], 
                                     bin_url = check_ck$bold_bin_uri[i]))
    }
}
names_changes <- unique(name_changes[name_changes$new_name != "",])
### Mel, it might be useful to double check this list of 25 name changes before moving on
names_changes
### Assuming they are right, I've replaced them in the bins table.
for(i in 1:nrow(bins)){
  bins$species[bins$species == names_changes$name_in_tfill[i]] <- names_changes$new_name[i]
}
### And the following adds new records to the bins table
for(i in 1:nrow(arc_unmatched_checked)){
  if(arc_unmatched_checked$checked_species_name[i] %in% bins$species){
    bini <- bins[bins$species == arc_unmatched_checked$checked_species_name[i],]
    if(nrow(bini) == 1){
      if(!is.na(bini$bin_uri) & is.na(arc_unmatched_checked$bold_bin_uri[i])){
        if(bini$bin_uri != arc_unmatched_checked$bold_bin_uri[i]){
          stop("1")
        }
      }
    }else{
      # If the bins table has an NA for this species, then assume that was because this match was missed
      # and remove the NA record
      if(sum(is.na(bini$bin_uri)) > 0){
        bins <- bins[!(bins$species == arc_unmatched_checked$checked_species_name[i] & is.na(bins$bin_uri)),]
        bini <- bins[bins$species == arc_unmatched_checked$checked_species_name[i],]
      }
      if(!arc_unmatched_checked$bold_bin_uri[i] %in% bins$bin_uri){
        bins <- rbind(bins, data.frame(species = arc_unmatched_checked$checked_species_name[i],
                                       bin_uri = arc_unmatched_checked$bold_bin_uri[i]))
      }else{
        # bin_uri is already listed so let it be
      }
    }
  }
}

### Remove duplicate bin records
bins <- unique(bins)

# Check for B- names needing changing
unq_bin_spp <- unique(bins$species)
name_changes_bins <- data.frame(existing_name = NA, suggested_new_name = NA)[0,]
for(i in 1:length(unq_bin_spp)){
  bini <- bins[bins$species == unq_bin_spp[i],]
  bini <- bini[!is.na(bini$bin_uri),]
  if(grepl("B-",unq_bin_spp[i])){
    if(nrow(bini) == 1 & 
       (grepl("/",unq_bin_spp[i]) | 
        grepl("group",unq_bin_spp[i]))){
      name_changes_bins <- rbind(name_changes_bins, 
                                 data.frame(existing_name = unq_bin_spp[i], 
                                            suggested_new_name = paste0(strsplit(unq_bin_spp[i], " ")[[1]][1],
                                                                        " sp. B-", bini$bin_uri)))
    }
    if(nrow(bini) == 2 & 
       !grepl("/",unq_bin_spp[i])){
       name_changes_bins <- rbind(name_changes_bins, 
                                 data.frame(existing_name = unq_bin_spp[i], 
                                            suggested_new_name = paste0(strsplit(unq_bin_spp[i], " ")[[1]][1],
                                                                        " sp. B-", bini$bin_uri[1], "/", bini$bin_uri[2])))
  }
    if(nrow(bini) > 2 & 
       !grepl("group",unq_bin_spp[i])){
       name_changes_bins <- rbind(name_changes_bins, 
                                 data.frame(existing_name = unq_bin_spp[i], 
                                            suggested_new_name = paste0(strsplit(unq_bin_spp[i], " ")[[1]][1],
                                                                        " sp. B-", bini$bin_uri[1], " group")))
  }
  }
}

### MEL, worth having a look through these suggested name changes.  (Particularly the ones where I have suggested dropping from a group or a "/" to a single bin_uri name.  Perhaps I am missing some important information in my input data?)
### If some of these are in error, we can fix the code and make these name changes automatically rather than manually.
name_changes_bins

bins_bu <- bins

### Check for non-B names that need changing
bins_non_bs <- bins[grepl("sp\\.", bins$species) & !grepl("B-", bins$species) & !is.na(bins$bin_uri),]
bins_non_bs_spp <- unique(bins_non_bs$species)
bins_non_bs$species_corrected <- NA
for(i in 1:length(bins_non_bs_spp)){
  bini <- bins[bins$species == bins_non_bs_spp[i],]
  if(nrow(bini) == 1){
    new_name <- paste0(strsplit(bins_non_bs_spp[i], " ")[[1]][1], " sp. B-", bini$bin_uri)
  }
  if(nrow(bini) == 2){
    new_name <- paste0(strsplit(bins_non_bs_spp[i], " ")[[1]][1], " sp. B-", bini$bin_uri[1], "/", bini$bin_uri[1])
  }
  if(nrow(bini) == 3){
    new_name <- paste0(strsplit(bins_non_bs_spp[i], " ")[[1]][1], " sp. B-", bini$bin_uri[1], " group")
  }
    bins_non_bs$species_corrected[bins_non_bs$species == bins_non_bs_spp[i]] <- new_name
    bins$species[bins$species == bins_non_bs_spp[i]] <- new_name
    tfill$species[!is.na(tfill$species) & tfill$species == bins_non_bs_spp[i]] <- new_name
    if(sum(arc_unmatched_checked$checked_species_name == bins_non_bs_spp[i]) > 0){
      stop("1")
    arc_unmatched_checked$checked_species_name[arc_unmatched_checked$checked_species_name == bins_non_bs_spp[i]] <- new_name
    }
    }

### Mel - you should check this table. Here I try to rename these 72 species more appropriately
### But once again, perhaps my rabbit hole work above could help with this...
bins_non_bs <- unique(bins_non_bs$species)

### Now I want to check for any remaining asv_codes that have no matching bin_uri, 
### but have been assigned species names that do have matches. (bin_check_nas)  Could these be errors?
### I also create vector of species names with no bin_uri at all (bin_single_nas).  
#### I think it is worth checking both these vectors of species names for potential naming errors in the first
#### instance.  
bin_single_nas <- vector()
bin_check_nas <- vector()
for(i in 1:length(unq_bin_spp)){
  bini <- bins[bins$species == unq_bin_spp[i],]
  if(nrow(bini) == 1  & sum(is.na(bini$bin_uri)) > 0){
    bin_single_nas <- c(bin_single_nas, unq_bin_spp[i])
  }
  if(nrow(bini) > 1  & sum(is.na(bini$bin_uri)) > 0){
    bin_check_nas <- c(bin_check_nas, unq_bin_spp[i])
  }
}
### Remove unidentified records
bin_single_nas <- bin_single_nas[!grepl("Unident", bin_single_nas)]
### Worth checking this full list
bin_single_nas
### I think that those with no matching bin but have been given a B- name need checking in particular!
bin_single_nas[grep("B-", bin_single_nas)]

### Here I find the asv_codes associated with bin_single_nas and bin_check_nas that returned  
### no BOLD matches in my original version 4 search
### Note that this is going to miss the non-aquatic non-macro records that remain in this set
### (But perhaps my rabbit hole work with your bold results files will help in that regard)
### Data loaded using line 28-42 in Appendix 2
load("data/asv_library_97_match_list_with_bin_uris_2024-12-15.rda")
bin_check_nas_asv <- data.frame(species = NA, asv_code = NA, asv_sequence = NA)[0,]
for(i in 1:length(bin_single_nas)){
  asvi <- all_arc$asv_code[all_arc$species == bin_single_nas[i]]
  seqi <- all_arc$asv_seq[all_arc$species == bin_single_nas[i]]
  if(length(asvi) == 0){
    asvi <- arc_unmatched_checked$asv_code[arc_unmatched_checked$checked_species_name == bin_single_nas[i]]
     seqi <- arc_unmatched_checked$asv_seq[arc_unmatched_checked$checked_species_name == bin_single_nas[i]]
  }
  if(length(asvi) > 0)
  bin_check_nas_asv <- rbind(bin_check_nas_asv, data.frame(species = bin_single_nas[i], 
                                                           asv_code = asvi, 
                                                           asv_sequence = seqi))
}
not_matched_v4 <- bin_check_nas_asv[0,]
not_checked_v4 <- bin_check_nas_asv[0,]
for(i in 1:nrow(bin_check_nas_asv)){
  if(!bin_check_nas_asv$asv_code[i] %in% names(match_list_bin_uri))
  {
    not_checked_v4 <- rbind(not_checked_v4, bin_check_nas_asv[i,])
  }else{
  matchi <- match_list_bin_uri[[bin_check_nas_asv$asv_code[i]]]
  if(nrow(matchi) == 0){
    not_matched_v4 <- rbind(not_matched_v4, bin_check_nas_asv[i,])
  }
  }
}

### Check none of these have already been checked in the arc and maxp tables (combined above)
sum(!not_matched_v4$asv_code %in% bold_ra_asvs) # 639
sum(!not_checked_v4$asv_code %in% bold_ra_asvs) # 71

results_all <- rbind(tfill, maxp)
results_omitted <- results_all[!results_all$asv_code %in% bold_ra_asvs,]

### Mel, I think it might be worth your while doing an additional search in BOLD v5 for 
### the 71 records that weren't checked in version 4 
### the 639 records that weren't matched in version 4 but were assigned a species name that did match (not_matched_v4[])

### Add all_arc_unmatched that were not checked and with max_p_identity >=97 , 
### which was saved as arc_unmatched_checked to tfill.
### First problem is that arc_unmatched_checked doesn't have amplicon, class or kingdom fields
### For now, I will create two empty fields, but you might need to go back and dig up these values from source
arc_unmatched_checked$amplicon <- NA
arc_unmatched_checked$kingdom <- NA
arc_unmatched_checked$class <- NA
names(arc_unmatched_checked)[names(arc_unmatched_checked) == "asv_seq"] <- "asv_sequence"
### make species field = species_orig, except where arc_unmatched_checked has identified a new species_bold name
arc_unmatched_checked$species <- arc_unmatched_checked$species_orig
for(i in 1:nrow(arc_unmatched_checked)){
    if(!(arc_unmatched_checked$checked_species_name[i] %in% c("",arc_unmatched_checked$species[i]))){
      arc_unmatched_checked$species[i] <- arc_unmatched_checked$checked_species_name[i]
      # A couple of cases where genus name is changed with new bold name
      if(arc_unmatched_checked$genus[i] != strsplit(arc_unmatched_checked$species[i]," ")[[1]][1]){
        arc_unmatched_checked$genus[i] <- strsplit(arc_unmatched_checked$species[i]," ")[[1]][1]
      }}
}
### Now Mel, I don't get which of these should be added to tfill...just those with max_p_identity >= 97?
### The next commented out line is what needs to be done, but I think arc_unmatched_checked needs to be subsetted first, no?
# tfill <- rbind(tfill, arc_unmatched_checked[match(names(tfill), names(arc_unmatched_checked))])
...
### So am I right in thinking we are only adding to the asv_library, not making corrections to identifications derived in appendix 2? (Head spinning a bit now. Probably need a rest...Head still spinning on day 2...I feel like I want to pull this all apart and start again...)
