...
mw_mw46 <- read.csv(here::here("results/final_data_summaries_3_amp/MW_data_vsearch_filled_summary_25_06_2025.csv"))
```
...
mw_mw46_fill <- bind_rows(df1_final, df3) %>%
...
```{r}
# These two lines split the data based on whether max_p_identity and species names are present or missing
# Consider a record filled only if species is assigned and max_p_identity >= 97
mw_mw46_filled1 <- mw_mw46_fill %>%
  filter(!is.na(max_p_identity) & max_p_identity >= 97 & species != "") %>% select(asv_code, kingdom, phylum, class, order, family, genus, species, Consensus, max_p_identity, threshold, everything()) %>%
  relocate(asv_seq, amplicon, .after = last_col()) #3184

# Records not yet filled (e.g., species is missing OR identity is too low or missing)
mw_mw46_not_filled <- mw_mw46_fill %>%
  filter(is.na(max_p_identity) | max_p_identity < 97 | species == "") #2310
...
bold_results1 <- read.csv(here::here("BOLD_asv_searches/MW46Barcode ID - BOLD - db_all1_1000.csv"))
bold_results2 <- read.csv(here::here("BOLD_asv_searches/MW46Barcode ID - BOLD - db_all1001_2000.csv"))
bold_results3 <- read.csv(here::here("BOLD_asv_searches/MW46Barcode ID - BOLD - db_all2001_end.csv"))
bold_results4 <- read.csv(here::here("BOLD_asv_searches/BOLD_mw46_3_missed.csv"))
...
# 
# asv_tax_all <- asv_tax_all %>%

# read in taxa not in asv library
asv_missing_tax <- read.csv(here::here("BOLD_asv_searches/asv_missing_tax.csv"))
...
# can't get these to update using df
bold_results_sum <- bold_results_sum %>%
  mutate(
    species = case_when(
      asv_code == "e7274a63ed58d5004381d9374d32c1ed" ~ "Garrha sp. B-ABU9552",
      TRUE ~ species),
    genus = case_when(
      asv_code == "e7274a63ed58d5004381d9374d32c1ed" ~ "Garrha",
      TRUE ~ genus))
      
bold_results_sum <- bold_results_sum %>% select(asv_code, kingdom, phylum, class, order, family, genus, species, max_p_identity) #933 results updated
```
...
mw_mw46_not_filled2 <- mw_mw46_not_filled %>% filter(!asv_code %in% common_ids)
...
mw_mw46_filled2 <- mw_mw46_filled2 %>%
  select(asv_code, kingdom, phylum, class, order, family, genus, species, Consensus, max_p_identity, threshold, everything()) %>%
  relocate(asv_seq, amplicon, .after = last_col())
```

```{r}
#write.csv(mw_mw46_not_filled2,here::here("BOLD_asv_searches/mw_mw46_still_not_filled3.csv" ))
```
...
mw_mw46_priv_search <- read.csv(here::here("geneious_search/geneious_mw46mw_not_bold_private.csv"))
...
# STEP 2: Add fallback taxonomy info (with your rules)
mw_mw46_priv_search_gt97 <- mw_mw46_priv_search %>%
  mutate(
    fallback_kingdom = "Eukaryota",
    fallback_phylum = if_else(grepl("Caenidae|Notonemouridae", species), "Arthropoda", NA_character_),
    fallback_class  = if_else(grepl("Caenidae|Notonemouridae", species), "Insecta", NA_character_),
    fallback_order  = case_when(
      grepl("Caenidae", species) ~ "Ephemeroptera",
      grepl("Notonemouridae", species) ~ "Plecoptera",
      TRUE ~ NA_character_
    ),
    fallback_family = case_when(
      grepl("Caenidae", species) ~ "Caenidae",
      grepl("Notonemouridae", species) ~ "Notonemouridae",
      TRUE ~ NA_character_
    ),
    fallback_genus = ""
  )
...
mw_mw46_priv_search_gt97 <- mw_mw46_priv_search_gt97 %>%
  ...
  mutate(
    kingdom = coalesce(kingdom, fallback_kingdom),
    phylum  = coalesce(phylum,  fallback_phylum),
    class   = coalesce(class,   fallback_class),
    order   = coalesce(order,   fallback_order),
    family  = coalesce(family,  fallback_family),
    genus   = coalesce(genus,   fallback_genus)
  ) %>%
...
mw_mw46_not_filled3 <- mw_mw46_not_filled2 %>% filter(!asv_code %in% common_ids)
...
```{r}
gb_search_results <- read.csv(here::here("gb_searches/MW46_genbank search.csv"))
...
mw_mw46_filled4 <- bind_rows(df1_updated, df3)

mw_mw46_filled4 <- mw_mw46_filled4 %>%
 select(asv_code, kingdom, phylum, class, order, family, genus, species, Consensus, max_p_identity, threshold, everything()) %>%
  relocate(asv_seq, amplicon, .after = last_col())
```
...
mw_final_df1_filled <- rbind(mw_mw46_filled1, mw_mw46_filled2, mw_mw46_filled3, mw_mw46_filled4)
...
file_name <- here::here("results/final_data_summaries", paste0("MW_data_vsearch_filled_summary_", timestamp, ".csv"))
...
mw_final_df2  <- read.csv("~/git/metabarcoding_workflow_MW_reruns/results/final_data_summaries_3amp/MW_vsearch_data_summary_re-reruns_18_06_2025.csv")
```
...
mw_final_df2_not_filled <- mw_final_df2_not_filled %>%
  mutate(max_p_identity = as.numeric(max_p_identity))

# Step 6: Recombine with the rest of the final_df that had no update
# Start with df1r_updated and assign to mw_mw46_fill2
mw_mw46rr_filled1 <- df1r_updated %>% select(asv_code, kingdom, phylum, class, order, family, genus, species, Consensus, max_p_identity, threshold, everything()) %>%  select(-asv_sequence) # 174 filled
```

# Finding any new asvs in re-runs and adding classification
```{r}
#write.csv(mw_final_df2_not_filled, here::here("geneious_search/mw_final_df2_not_filled.csv"))
...
rr_bold_results2 <- read.csv(here::here("BOLD_asv_searches/BOLD_mw46_3rr_missed.csv"))
...
rr_bold_results_sum <- rr_bold_results_sum %>%
  ...
  dplyr::rename(
    species = species_bold,) %>% 
  mutate(kingdom = "Eukaryota") %>%  # Add 'kingdom' column filled with NA (character)
  relocate(kingdom, .before = phylum) %>% # Move 'kingdom' before 'phylum'
  filter(asv_code != "cb3c8c0e3c630e9daddafc4ca759a4c9") %>%
  mutate(species = ifelse(asv_code == "98bd7821b7f71ac510b40602e1312232", "Neureclipsis napaea", species))
...
# 1. Join on asv_code
df_joined <- mw_final_df2_not_filled %>%
  left_join(
    rr_bold_results_sum %>%
      select(asv_code, kingdom, phylum, class, order, family, genus, species, max_p_identity),
    by = "asv_code",
...
mw_mw46_rr_filled2 <- mw_mw46_rr_filled2 %>% filter(max_p_identity >= 97) #44
```


```{r}
mw_final_df2_not_filled2 <- mw_final_df2_not_filled %>%
  filter(!asv_code %in% mw_mw46_rr_filled2$asv_code)

#write.csv(mw_final_df2_not_filled2, here::here("geneious_search/mw_fina3l_df2_not_filled.csv"))

# private lib search

#add in private library
priv_search_rr <- read.csv(here::here("geneious_search/geneious_mw46mw_rr3_not_bold_private.csv"))

# STEP 1: Preprocess and filter
priv_search_rr_gt97 <- priv_search_rr %>%
  dplyr::rename(
    max_p_identity = X..Pairwise.Identity,
    coverage = Query.coverage,
    asv_code = Query
...
```{r}
mw_final_df2_filled <- rbind(mw_mw46rr_filled1, mw_mw46_rr_filled2, mw_mw46_rr_filled3)
```
...
file_name <- here::here(paste0("results/final_data_summaries/MW_data_vsearch_filled_summary_reruns_", timestamp, ".csv"))
...
```{r}
#write.csv(new_asv_for_asv_lib, "~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_library/new_asv_for_asv_lib.csv")
```
