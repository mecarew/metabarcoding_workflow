...
##Add new data 30_Oct_2024
```{r}
# read in flies
file_path <- "~/Documents/additions_asv_library_oct24.xlsx"

# Get the names of the sheets
sheet_names <- readxl::excel_sheets(file_path)

# Read the sheets into as dataframes
truncated_fills_add_df <- readxl::read_excel(file_path, sheet = sheet_names[1])  # contains full taxonomic information for classifications with truncated taxonomy

# Add new data to original data
truncated_fills_df <- rbind(truncated_fills_df, truncated_fills_add_df)
```


```{r}
# read in flies
truncated_fills_miseq15_20_df <- read.csv("~/Documents/asv_library_97_add_miseq15-20_records.csv")

# Add new data to original data
truncated_fills_df <- rbind(truncated_fills_df, truncated_fills_add_df,truncated_fills_miseq15_20_df)
```
...
truncated_fills_df <- truncated_fills_df %>%
  mutate(species = gsub("Aedes alboannulotus", "Aedes alboannulatus", species),
         ...
         species = gsub("Notalina bifara complex B-ABV8023", "Notalina bifaria B-ABV8023 group", species),
         species = gsub("Musculium sp. B-ADJ8477", "Sphaerium sp. B-ADJ8477", species),
         species = gsub("GB-", "G-", species),
         species = gsub("ZB-", "", species),
         species = gsub("complex", "group", species),
         species = gsub("  ", " ", species),
         species = gsub("Cricptopus", "Cricotopus", species),
         species = gsub("Asmicridea edwardsiii", "Asmicridea edwardsii group", species),
         species = gsub("Chironomidae sp. B-ABX4803", "Chironominae sp. B-ABX4803", species),
         species = gsub("Chironomidae sp. B-AEE9278", "Orthocladiinae sp. B-AEE9278", species),
         species = gsub("Dero obtusa", "Dero obtusa/nivea", species),
         species = gsub("Atalophlebia sp. B-ACM3545", "Atalophlebia sp. B-ABV8729 group", species),
         species = gsub("Lingora aurata", "Lingora aurata group", species),
         species = gsub("Dinotoperla aff. thwaitesi", "Dinotoperla thwaitesi group", species),
         species = gsub("austrosp", "Trinotoperla sp. B-ABX3905 group", species),
         species = gsub("Dero obtusa", "Dero obtusa/nivea", species),
         species = gsub("Nousia sp. B-ACM3590", "Nousia sp. B-AAV4108 group", species),
         species = gsub("Simuliidae sp. B-AEV0976", "Simuliidae sp. B-AEV0976/AEV6641", species),
         species = gsub("Atalophlebia sp. B-ACM3545 group", "Atalophlebia sp. B-ABV8729 group", species),
         species = gsub("Centroptilum sp. B-ACM3150", "Centroptilum elongatum", species),
         species = gsub("Hydraenidae sp. B-ADK5039", "Gymnochthebius australis", species))
...
max_p_update_df <- max_p_update_df %>%
  mutate(species = gsub("Aedes alboannulotus", "Aedes alboannulatus", species),
         ...
         species = gsub("  ", " ", species),
         species = gsub("Cricptopus", "Cricotopus", species),
         species = gsub("sp.B", "sp. B", species),
         species = gsub("Asmicridea edwardsiii", "Asmicridea edwardsii group", species),
         species = gsub("Chironomidae sp. B-ABX4803", "Chironominae sp. B-ABX4803", species),
         species = gsub("Chironomidae sp. B-AEE9278", "Orthocladiinae sp. B-AEE9278", species),
         species = gsub("Dero obtusa", "Dero obtusa/nivea", species),
...
# corrections of records with incorrect species names in tfill spreadsheet
```{r}
...
"8bdc2627bd8205a94f40a831d20e1aef" = "Riethia sp. B-ACG9457/ADU5431",
...
"1d5760deee25f457d5f3aae91017277a" = "Riethia sp. B-ACG9457/ADU5431",
"f0671dc5cb44b617842a1e84dba330c0" = "Tanytarsus fuscithorax",
"7bffac2affe628b01910829ec8b4c176" = "Centroptilum elongatum",
"741aca4ed6f6847e894d0ae837cc5032" = "Psychoda gemina",
"e7f87bed531e9a3057d3abbc7b296f9f" = "Sigara truncatipala",
"5b0c60ab95cf5013926f4afdd2815273" = "Sigara truncatipala")

# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(species = ifelse(asv_code %in% names(asv_species_corrections_tf),
                          asv_species_corrections_tf[asv_code],
                          species))

```

# corrections of records with incorrect genus names in tfill spreadsheet
```{r}
...
# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(genus = ifelse(asv_code %in% names(asv_genus_corrections_tf),
                          asv_genus_corrections_tf[asv_code],
                          genus))

```

# corrections of records with incorrect family names in tfill spreadsheet
```{r}
...
# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(family = ifelse(asv_code %in% names(asv_family_corrections_tf),
                          asv_family_corrections_tf[asv_code],
                          family))

```


# corrections of records with incorrect order names in tfill spreadsheet
```{r}
asv_order_corrections_tf <- c("9ca9760013a77462d9c5e2c942e4d27e" = "Peronosporales")

# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(order = ifelse(asv_code %in% names(asv_order_corrections_tf),
                          asv_order_corrections_tf[asv_code],
                         order))
```

# corrections of records with incorrect class names in tfill spreadsheet
```{r}
asv_class_corrections_tf <- c("9ca9760013a77462d9c5e2c942e4d27e" = "Oomycota")

# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(class = ifelse(asv_code %in% names(asv_class_corrections_tf),
                          asv_class_corrections_tf[asv_code],
                         class))
```

# corrections of records with incorrect phylum names in tfill spreadsheet
```{r}
asv_phylum_corrections_tf <- c("9ca9760013a77462d9c5e2c942e4d27e" = "Heterokontophyta")

# Update the species column based on asv_code matching the correction vector
truncated_fills_df <- truncated_fills_df %>%
  mutate(phylum = ifelse(asv_code %in% names(asv_phylum_corrections_tf),
                          asv_phylum_corrections_tf[asv_code],
                         phylum))
```
...
# add corrections to complexes (groups) to complexes_df
truncated_fills_df <- truncated_fills_df %>%
  mutate(species = gsub("complex", "group", species))
      
```

# delete misidentified records in in tfill spreadsheet
```{r}
truncated_fills_df <- truncated_fills_df %>%
...
# corrections of records with incorrect species names in max_p_update spreadsheet
```{r}
...
max_p_update_df <- max_p_update_df %>%
  mutate(species = ifelse(asv_code %in% names(asv_species_corrections_mpu),
                          asv_species_corrections_mpu[asv_code],
                          species))

```

# corrections of records with incorrect genus names in max_p_update spreadsheet
```{r}
asv_genus_corrections_mpu <- c("a3b763c11784ab20ccb6053c998cd0e4" = "Ulmerophlebia")

max_p_update_df <- max_p_update_df %>%
  mutate(genus = ifelse(asv_code %in% names(asv_genus_corrections_mpu),
                          asv_genus_corrections_mpu[asv_code],
                          genus))
```

# corrections of records with incorrect family names in max_p_update spreadsheet
```{r}
asv_family_corrections_mpu <- c("a3b763c11784ab20ccb6053c998cd0e4" = "Leptophelbiidae")

max_p_update_df <- max_p_update_df %>%
  mutate(family = ifelse(asv_code %in% names(asv_family_corrections_mpu),
                          asv_family_corrections_mpu[asv_code],
                          family))
```

# corrections of records with incorrect max_p_identity names in max_p_update spreadsheet
```{r}
asv_max_p_identity_corrections_mpu <- c("a3b763c11784ab20ccb6053c998cd0e4" = "97")

max_p_update_df <- max_p_update_df %>%
  mutate(max_p_identity = ifelse(asv_code %in% names(asv_max_p_identity_corrections_mpu),
                          asv_max_p_identity_corrections_mpu[asv_code],
                          max_p_identity))
```
...
"Pisidium sp. B-AAG0350/ACQ7011",
"Pisidium sp. B-AAG0350",
"Pisidium sp. B-ACQ7011",
"Pisidium casertanum",
"Pisidium fallax",
"Pisidium supinum",
"Pisidium sp. B-ADI7406",
"Pisidium sp. BAEB7951",
"Hydra vulgaris",
...
"Limnophila sp. ZB-EPAsp30",
"ParaLimnophila sp. B-private",
"Paralimnophila sp. B-ACM3249",
"Paralimnophila sp. B-ACM3182",
"Tonnoiromyia sp. ZB-EPAsp3",
"Austrochiltonia sp. MC1",
"Nais sp. B-ACB6480",
"Limnophila sp. MC-1",
"Lumbriculus sp. B-AEC5767",
"Lingora sp. AV1",
"Lingora sp. B-ABY8640/AAX5053",
"Lingora sp. B-AAY5550/ADF2103",
"Lingora sp. B-ACC8619",
"Dinotoperla walkeri",
"Dinotoperla evansi",
"Atalophlebia sp. B-ABX7606",
"Atalophlebia sp. B-ACM3545",
"Atalophlebia sp. B-ABV8739",
"Atalophlebia sp. B-ABV8733",
"Cognettia sphagnetorum",
"Cognettia chlorophila",
"Cryptochironomus sp. B-ACS3851",
"Cryptochironomus sp. B-ACS4478/ACS3865",
"Ferrissia sp. B-ADR3550 group",
"Ferrissia sp. B-AAE6642",
"Koorrnonga sp. B-ABV9140", 
"Koorrnonga sp. B-ABV9142",
"Limnophila sp. B-ABX1094",
"Limnophila sp. B-ABX1091",
"Nais christinae",
"Nais variabilis",
"Nais elinguis",
"Nais pseudobtusa",
...
"Schizonyxhelea sp. B-ADTX1789",
"Schizonyxhelea sp. B-AFV6727",
"Schizonyxhelea sp. B-ABX1780",
"Tasmanocoenis sp. B-ACG1634",
"Ceratopogonidae sp. 12",
"Hydrophilidae sp. MC-5",
"Tipulidae sp. MC-6",
"Centroptilum sp. B-ACM3150",
"Micronecta sp. B-ACG1960",
"Micronecta sp. B-ADU6789",
"Riethia sp. B-ACG9457",
"Riethia sp. B-ADU5431",
"Apsectrotanypus sp. MC-4",
"Apsectrotanypus sp. MC-1",
"Austroargiolestes",
"Chaetogaster sp. CEO",
"Gyraulus sp. B-ACO7407",
"Hydra sp. MC-O1",
"Ilyodrilus sp. CEB",
"Nais sp. B-ACB6665",
"Dolichopodidae sp. B-AAW0925",
"Oxyethira sp. B-ACN2323",
"Physa acuta",
"Pristina sp. CEE",
"Pristina sp. CEF",
"Limnodrilus claparedeanus",
"Microvelia sp. B-ADL6764",
"Notalina bifaria",
"Notalina sp. B-ADF2209",
"Notalina sp. B-AAI2529",
"Polypedilum sp. MC-B",
"Psychodidae sp. B-AAU4648",
"Psychoda sp. B-AAU4648",
"Notalina parkeri")
...
"Nousia sp. B-AAV4106 group",
"Nousia sp. B-AAV4106 group",
"Polypedilum sp. B-ABW9978 group",
"Polypedilum sp. B-ABW9978 group",
"Polypedilum sp. B-ABW9978 group",
"Polypedilum sp. B-ABW9978 group",
"Polypedilum sp. B-ABW9978 group",
...
"Tamasia acuta/variegata",
"Tamasia acuta/variegata",
"Ferrissia sp. B-AAY1880 group",
"Ferrissia sp. B-AAY1880 group",
"Ferrissia sp. B-AAY1880 group",
"Zavrelimyia sp. B-AEN0522/AEN0524",
"Zavrelimyia sp. B-AEN0522/AEN0524",
"Zavrelimyia sp. B-ACQ7611 group",
...
"Scatella sp. B-ACK6288/ACZ6583",
"Scatella sp. B-ACK6288/ACZ6583",
"Sclerocyphon sp. G-C18 group",
"Sclerocyphon sp. G-C18 group",
"Sclerocyphon sp. G-C18 group",
"Sclerocyphon sp. G-C18 group",
"Pisidium sp. B-AAG0350 group",
"Pisidium sp. B-AAG0350 group",
"Pisidium sp. B-AAG0350 group",
"Pisidium sp. B-AAG0350 group",
"Pisidium sp. B-AAG0350 group",
...
"Australopelopia sp. B-AEN1251",
"Limnophila sp. EPAsp30",
"Paralimnophila sp. B-ACM3182/ACM3249",
"Paralimnophila sp. B-ACM3182/ACM3249",
"Paralimnophila sp. B-ACM3182/ACM3249",
"Tonnoiromyia tasmaniensis",
"Austrochiltonia sp. MC-1",
"Nais complex group",
"Limnophila sp. B-ABA7043",
"Lumbriculus variegatus",
"Lingora sp. B-AAY5550 group",
"Lingora aurata group",
"Lingora sp. B-AAY5550 group",
"Lingora sp. B-AAY5550 group",
"Dinotoperla walkeri/evansi",
"Dinotoperla walkeri/evansi",
"Atalophlebia sp. B-ABV8729 group",
"Atalophlebia sp. B-ABV8729 group",
"Atalophlebia sp. B-ABV8733/ABV8739",
"Atalophlebia sp. B-ABV8733/ABV8739",
"Cognettia chlorophila/sphagnetorum",
"Cognettia chlorophila/sphagnetorum",
"Cryptochironomus sp. B-ACS3865 group",
"Cryptochironomus sp. B-ACS3865 group",
"Ferrissia sp. B-AAY1880 group",
"Ferrissia sp. B-AAY1880 group",
"Koorrnonga sp. B-ABV9140/ABV9142", 
"Koorrnonga sp. B-ABV9140/ABV9142",
"Limnophila sp. B-ABX1091/ABX1094",
"Limnophila sp. B-ABX1091/ABX1094",
"Nais communis group",
"Nais communis group",
"Nais elinguis/pseudobtusa",
"Nais elinguis/pseudobtusa",
"Rheumatometra dimorpha/philarete",
"Rheumatometra dimorpha/philarete",
"Schizonyxhelea sp. B-ABX1780 group",
"Schizonyxhelea sp. B-ABX1780 group",
"Schizonyxhelea sp. B-ABX1780 group",
"Tasmanocoenis tillyardi/rieki",
"Ceratopogonidae sp. MC-12",
"Enochrus sp. MC-5",
"Limoniidae sp. B-AGD4712/AGD4715",
"Centroptilum elongatum",
"Micronecta sp. B-ACG1960/ADU6789",
"Micronecta sp. B-ACG1960/ADU6789",
"Riethia sp. B-ACG9457ADU5431",
"Riethia sp. B-ACG9457ADU5431",
"Apsectrotanypus sp. B-AEM4866",
"Apsectrotanypus sp. B-ACD5991",
"Austroargiolestes sp. B-ACL1904 group",
"Chaetogaster sp. B-AEW4333",
"Gyraulus chinensis",
"Hydra vulgaris group",
"Ilyodrilus sp. B-AEV3713",
"Nais communis group",
"Nothorhaphium aemulans",
"Orthotrichia sp. B-ACN2323",
"Physella acuta",
"Pristina sp. B-AEW1288",
"Pristina sp. B-AEW1289",
"Limnodrilus claparedianus",
"Microvelia sp. B-ADH7670",
"Notalina bifaria B-ABV8023 group",
"Notalina bifaria B-ABV8023 group",
"Notalina bifaria B-ABV8023 group",
"Polypedilum sp. B-AAV5945",
"Psychoda gemina",
"Psychoda gemina",
"Notalina fulva/parkeri")
...
# change complex to group to reflect grouping are based on DNA metabarcoding analysis not taxonomic. Also add changes to grouping based on revised groupings
complexes_df <- complexes_df %>%
  mutate(
    complex = gsub("complex", "group", complex),
    complex = gsub("Cryptochironomus sp. B-ACS4478/ACS3865", "Cryptochironomus sp. B-ACS3865 group", complex),
    complex = gsub("Atalophlebia sp. B-ABV8733/ABV8739", "Atalophlebia sp. B-ABV8729 group", complex),
    complex = gsub("Lingora sp. B-AAY5550/ADF2103", "Lingora sp. B-AAY5550 group", complex),
    complex = gsub("Zavrelimyia sp. B-AEM5402 group", "Zavrelimyia sp. B-ACQ7611 group", complex),
    complex = gsub("Zavrelimyia sp. B-AEN0860 group", "Zavrelimyia sp. B-ACQ7611 group", complex),
    complex = gsub("Dero obtusa/nivea/nivea", "Dero obtusa/nivea", complex),
    complex = gsub("Nais group group", "Nais communis group", complex),
    complex = gsub("Notalina bifara group B-ABV8023", "Notalina bifaria B-ABV8023 group", complex),
    complex = gsub("Notalina B-ABV8023 group", "Notalina bifaria B-ABV8023 group", complex),
    complex = gsub("Trinotoperla sp. group B-ABX3905", "Trinotoperla sp. B-ABX3905 group", complex),
    complex = gsub("Notalina sp. B-AAX5636", "Notalina fulva/parkeri", complex))
```
...
Hydrobiosella waddama cf4d8b16cb64eb29dd6eb8aeff774878 

Given that metabarcoding can introduce errors in and this may be the cause of these boarderline overlaps. These BIN's will not be combined at this stage

#Taxonomic errors on databases
BIN - AAX5275  It matches two different species in different genera/species. I suspect the Limnodrilus hoffmeisteri is the incorrect identification and identification of Bothrioneurum vejdovskyanum is correct. This is due to Bothrioneurum vejdovskyanum being identified by a taxonomic expert. Limnodrilus hoffmeisteri has many other records not associated with the BIN and the sequence was mined from GenBank
...
# Save the workbook
saveWorkbook(wb, "~/Documents/asv_library_97_aug24_modified.xlsx", overwrite = TRUE)
```

## comments of changes 10_Dec_24

<!-- tfill$species <- gsub("  ", " ", tfill$species) added -->
<!-- tfill$species <- gsub("Cricptopus", "Cricotopus", tfill$species) added -->
<!-- tfill$species <- gsub("sp.B", "sp. B", tfill$species) added -->

<!-- #Inconsistent naming? -->
<!-- tfill$species[tfill$species == "Asmicridea edwardsiii"] <- "Asmicridea edwardsii group" added -->
<!-- tfill$species[tfill$species == "Chironomidae sp. B-ABX4803"] <- "Chironominae sp. B-ABX4803" added -->
<!-- tfill$species[tfill$species == "Chironomidae sp. B-AEE9278"] <- "Orthocladiinae sp. B-AEE9278" added -->
<!-- tfill$species[tfill$species == "Dero obtusa"] <- "Dero obtusa/nivea" added -->
<!-- tfill$species[tfill$species == "Atalophlebia sp. B-ACM3545"] <- "Atalophlebia sp. B-ABV8729 group" added -->
<!-- tfill$species[tfill$species == "Lingora aurata"] <- "Lingora aurata group" added -->

<!-- tfill$species <- gsub("Dinotoperla aff. thwaitesi", "Dinotoperla thwaitesi group", tfill$species) added -->
<!-- tfill$species <- gsub("Dinotoperla walkeri/evansi", "Dinotoperla walkeri", tfill$species) changed Dinotoperla walkeri to Dinotoperla walkeri/evansi and added to complexes table -->
<!-- tfill$species <- gsub("Dinotoperla aff. thwaitesi", "Dinotoperla thwaitesi group", tfill$species)? Is this the same as above? -->
<!-- tfill$species <- gsub("Pisidium sp. cf. hallae group","Pisidium sp. B-AAG0350/ACQ7011", tfill$species) seems like there are many BINs/species that overlap. I have added all these to the complexes table and now called them “Pisidium sp. B-AAG0350 group” as the “Pisidium sp. cf. hallae” name is not used by BOLD anymore -->
<!-- tfill$species <- gsub("Trinotoperla sp. group B-ABX3905","Trinotoperla sp. B-ABX3905 group",tfill$species) added -->
<!-- tfill$species <- gsub("Austropyrgus centralia group","Austropyrgus centralia/turbatus",tfill$species) changed Austropyrgus centralia/turbatus to Austropyrgus centralia group (in ‘complexes’ too) and added additional species that match this group -->
<!-- tfill$species <- gsub("Nousia sp. B-ACM3590", "Nousia sp. B-AAV4108 group",tfill$species) added and also added Nousia sp. B-ACM3590 to Nousia sp. B-AAV4108 group  in ‘complexes’ -->
<!-- tfill$species <- gsub("Simuliidae sp. B-AEV0976", "Simuliidae sp. B-AEV0976/AEV6641",tfill$species) added already in ‘complexes’ so picked up there but added anyway -->
<!-- tfill$species <- gsub("Trinotoperla sp. B-ABX3937","Trinotoperla sp. B-ABX3905 group",tfill$species) have not added. When check on BOLD id engine it suggests Trinotoperla sp. B-ABX3937 does not overlap with the Trinotoperla sp. B-ABX3905 group? -->


The bit of the script looking for pot_complexes turned up a few groups worth looking into further, I think.  (pot_complexes saved a csv file at "~/uomShare/wergStaff/ChrisW/temp/pot_complexes.csv"). Here's what I think, working down the not particularly long table:
AAX5275 - is shared across two species of different genera and an unidentified Clitellata specimen (asv_code 274d742e3ee489202dab4da510b8787a). I suspect this is one you've decided is a problem with dodgy BIN codes. If that is the case, we should document that in this qmd file. It matches two different species in different genera. I suspect the Limnodrilus hoffmeisteri is the incorrect identification and identification of Bothrioneurum vejdovskyanum is correct. This is due to Bothrioneurum vejdovskyanum being identified by a taxonomic expert. Limnodrilus hoffmeisteri has many other records not associated with the BIN and the sequence was mined from GenBank  I have added this correct the asv_correction_library.Rmd.
Atalophlebia sp. B-ABV8729 group and Atalophlebia sp. B-ABV3545 group overlap a lot. Combine? Atalophlebia sp. B-ABV8733 and Atalophlebia sp. B-ABV8739 overlap. Combine? Let’s combine I have changed ‘complexes’ so all fall under the name Atalophlebia sp. B- ABV8729 group.
- should it just be Centroptilum elongatum or is Centroptilum sp. B-ACM3150 really a separate species or should they be a group? same, added name correction Centroptilum sp. B-ACM3150 = Centroptilum elongatum 
AAT8936 - two separate genera ...another BOLD problem? (if so document decision) Cognettia sphagnetorum and Cognettia chlorophila all overlap. I have this to the ‘complexes’ table as Cognettia chlorophila/sphagnetorum. Records c2fb64f0e662b5db32c11ee187a9bd0f and c4e01d5fa4513318a1be2a00899d6e1a are not Chamaedrilus varisetosus they are Cognettia chlorophila/sphagnetorum. I have added these to the corrections to tfiil species and genus

ACV9482 - Cricotopus tasmania or a group with Cricotopus sp. B-ACQ6787? No. dfde7b1be822b7bc262c5802de893bef is Cricotopus tasmania not Cricotopus sp. B-ACQ6787. I have added these to the corrections to tfiil species.
ACS3851 - Cryptochironomus sp. B-ACS4478/ACS3865 seems an odd name for this group? There is some overlap between ACS3851/ACS4478/ACS3865. I’ll collapse these into a group. Now in ‘complexes’ spreadsheet
ACG2297 – Elmidae – some incorrect assignments of ACG2296 as ACG2297 changes added "7f15bdde33a25d1d333699264f073d37" = "Notriolus sp. B-ACG2297",
"f6ba88fdb937e5c233ff1acef9dc399b" = "Notriolus sp. B-ACG2297",
"7f15bdde33a25d1d333699264f073d37" = "Notriolus",
"f6ba88fdb937e5c233ff1acef9dc399b" = "Notriolus",

Ferrissia sp. B-AAE6642, Ferrissia sp. B-AAY1880 group, Ferrissia sp. B-ADR3550 group combine as groups overlap now called ‘Ferrissia sp. B-AAY1880 group’ all added to ‘complexes’

One incorrect Fridericia pretoriana  id ‘d54cf91de2a56cbde204c32eea369742" = "Fridericia sp. B-ADR5065”

One  incorrect ‘Galerucinae sp. B-ABX9821 id "2b943424b95aee577e27aa221cf57118" = "Galerucinae sp. B-ABA9962

Could not find an issue with Hydrobiosella perangusta and Hydrobiosella waddama seem to be distinct groups

Made new complex "Koorrnonga sp. B-ABV9140/ABV9142"

Corrected identifications for ADJ9699 and ADJ8523
“00b75095625dec00f84aaf4bd35f7561" = "Naididae sp. B-ADJ9699",
"8274abe141342580cb6f742678bb269f" = "Naididae sp. B-ADJ9699"
"c5f8109e46eebba439317d8eabd7c043" = "Naididae sp. B-ADJ9699",
"cfc29e6b343c7f58f485f311fa3cbb6e" = "Naididae sp. B-ADJ9699",
Limnophila sp. B-ABX1092 is Limnodrilus hoffmeisteri (mistake in code fixed) 
Lingora sp. B-AAY5550/ADF2103 add ACC8619 to complex new name Lingora sp. B-AAY5550 group
Nais variabilis and to Nais communis group

Nais pseudobtusa/ Nais elinguis new complex Nais elinguis/pseudobtusa

Some Psychoda gemina were Psychoda sp. B-AAU4648 record updated
...
"3b9fa04c8f620038fb9bace9f1512db2" = "Psychoda sp. B-AAU4648",
 ...
 New complex Rheumatometra dimorpha/philarete. Probably two species but too many discrepancies in naming to be sure so will combine.

Schizonyxhelea sp. B-ABX1780/ ADT1789/ AFV6727 new complex Schizonyxhelea sp. B-ABX1780 group

Simuliidae sp. B-AEV0976/AEV6641 already in complexes

Tanytarsus sp. B-AAF0821 = Tanytarsus fuscithorax
"f0671dc5cb44b617842a1e84dba330c0" = "Tanytarsus fuscithorax"

Trinotoperla sp. B-ABX3905 = Trinotoperla sp. B-ABX3905 group added to complexes

Agreed Zavrelimyia is a nightmare. Decided to lump to two groups Zavrelimyia sp. B-AEN0522/AEN0524 (seems distinct from others) and everything else is Zavrelimyia sp. B-ACQ7611 group
