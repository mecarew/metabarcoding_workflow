...
#load(paste0(uomshare, "results/cache/03.RData"))

# load thresholds_list form cahche03 
# thresholds_list <- readRDS(paste0(uomshare, "results/cache/thresholds_list.rds"))
# samples_df <- readRDS(paste0(uomshare, "results/cache/samples_df.rds"))
```

# load results files from previuos steps
These tables are required for assembling final data table
```{r}
left_lulu <- readRDS(paste0(uomshare, "results/lulu/left_lulu.rds"))
right_lulu <- readRDS(paste0(uomshare, "results/lulu/right_lulu.rds"))
long_lulu <- readRDS(paste0(uomshare, "results/lulu/long_lulu.rds"))
# left_classifications <- qiime2R::read_qza(paste0(uomshare, "results/classification/left_classification.qza"))
# right_classifications <- qiime2R::read_qza(paste0(uomshare, "results/classification/right_classification.qza"))
# long_classifications <- qiime2R::read_qza(paste0(uomshare, "results/classification/long_classification.qza"))
left_seq <- qiime2R::read_qza(paste0(uomshare, "results/dada2/left_representative_sequences.qza"))
right_seq <- qiime2R::read_qza(paste0(uomshare, "results/dada2/right_representative_sequences.qza"))
long_seq <- qiime2R::read_qza(paste0(uomshare, "results/dada2/long_representative_sequences.qza"))
```
...
left_search_results <- readr::read_tsv(paste0(uomshare, "results/classification/preliminary/left_vsearch.tsv"))
```

```{r}
# # load classification results for right amplicon
right_search_results <- readr::read_tsv(paste0(uomshare, "results/classification/preliminary/right_vsearch.tsv"))
```

```{r}
# # load classification results for right amplicon
long_search_results <- readr::read_tsv(paste0(uomshare, "results/classification/preliminary/long_vsearch.tsv"))
```
...
dir.create(paste0(uomshare, "results/final_data_summaries"))

timestamp <- format(Sys.time(), "%d_%m_%Y")

# Create the file name with the timestamp 
file_name <- paste0(paste0(uomshare, "results/final_data_summaries/MW_vsearch_data_summary_"), timestamp, ".csv")
