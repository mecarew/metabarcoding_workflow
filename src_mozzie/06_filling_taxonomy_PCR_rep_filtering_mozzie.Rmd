...
file_name <- paste0("~/git/metabarcoding_workflow_Miseq16/results/Miseq16_final_data_summary_filled_", timestamp, ".csv")
...
metab_long <- final_data_filled %>%
  pivot_longer(cols = Cont1:S21WAY30960rep3, names_to = "sample", values_to = "value") %>%
...
final_data_filled_clean <- final_data_filled %>%
  mutate(across(Cont8:S21WAY30960rep3, ~ replace_na(.x, 0)))  # Replace NA with 0 in specified columns

# Step 2: Pivot longer
metab_long <- final_data_filled_clean %>%
  pivot_longer(
    cols = Cont8:S21WAY30960rep3,  # Specify the range or individual column names
    names_to = "sample",
    values_to = "value"
  ) %>%
...
file_name <- paste0("~/git/metabarcoding_workflow_Miseq16/results/Miseq16_final_filt_filled_PCR_rep_data_summary_", timestamp, ".csv")
