...
file_name <- paste0("~/git/metabarcoding_workflow/results/MiSeq13_final_data_summary_filled_", timestamp, ".csv")
```
...
Note: This code will need to be configured for each dataset, so that the rows containing the read counts are specified (e.g. pivot_longer(cols = X10BNY29042rep1:Cont3rep1), the 'X10BNY29042rep1:Cont3rep1' will need to be changed to the first and last column name with the reads data
```{r}
...
```
# save the filtered filled dataframe with the PCR replcate seperate
```{r}
# save dataframe of filtered and filled data with PCR replicate 
timestamp <- format(Sys.time(), "%d_%m_%Y")

# Create the file name with the timestamp
file_name <- paste0("~/git/metabarcoding_workflow/results/MiSeq13_final_filt_filled_PCR_rep_data_summary_", timestamp, ".csv")
...
# Convert specified columns to character type
wide_summarised_site_per_df[cols_to_modify] <- lapply(wide_summarised_site_per_df[cols_to_modify], as.character)
...
wide_summarised_df <- summarized_df %>%
...
# Create the file name with the timestamp (NOTE:Change the MiSeq number to the dataset you are working with)
 wide_summarised_df <- paste0("~/git/metabarcoding_workflow/results/MiSeq13_final_data_summary_filled_filter_sites_", timestamp, ".csv", row.names = FALSE)
 
 write.csv(final_df, file =  wide_summarised_df, row.names = FALSE)
```
 
 
# FOR 10 STIE STUDY (with site_per) 
```{r}
summarized_site_per_df <- filtered_dataframe %>%
...
# Replace NA values with empty strings in specified columns
wide_summarised_site_per_df[cols_to_modify][is.na(wide_summarised_site_per_df[cols_to_modify])] <- ""

# Pivot wider to transform the site names into columns
wide_summarised_site_per_df <- summarized_site_per_df %>%
...
# Create the file name with the timestamp (NOTE:Change the MiSeq number to the dataset you are working with)
wide_summarised_site_per_df <- paste0("~/git/metabarcoding_workflow/results/MiSeq13_final_data_summary_filled_filter_sites_per_", timestamp, ".csv", row.names = FALSE)

write.csv(final_df, file =  wide_summarised_site_per_df, row.names = FALSE)
```