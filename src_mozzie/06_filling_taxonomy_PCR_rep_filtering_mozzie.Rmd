...
file_name <- paste0("~/git/metabarcoding_workflow_Miseq15/results/MiSeq15_final_data_summary_filled_", timestamp, ".csv")
...
metab_long <- final_data_filled %>%
  pivot_longer(cols = X10FEH41841rep3:S21YAR36377rep3, names_to = "sample", values_to = "value") %>%
...
counts <- filtered_dataframe_top_sp %>%
...
sp_fil_metab_long_rem <- filtered_dataframe_top_sp %>% filter((species %in% counts$species[counts$count == 1] & site %in% counts$site[counts$count == 1]))
...
# Step 3: Filter out reads below 10 for the top 10 species in filtered_dataframe
filtered_dataframe_top <- filtered_dataframe %>%
  filter(!(species %in% top_species & reads < 10))

# Verify the number of records before and after filtering
original_count <- nrow(filtered_dataframe)
filtered_count <- nrow(filtered_dataframe_top)
...
final_data_filled_clean <- final_data_filled %>%
  mutate(across(X10FEH41841rep3:S21YAR36377rep3, ~ replace_na(.x, 0)))  # Replace NA with 0 in specified columns

# Step 2: Pivot longer
metab_long <- final_data_filled_clean %>%
  pivot_longer(
    cols = X10FEH41841rep3:S21YAR36377rep3,  # Specify the range or individual column names
    names_to = "sample",
    values_to = "value"
  ) %>%
...
file_name <- paste0("~/git/metabarcoding_workflow_Miseq15/results/MiSeq15_final_filt_filled_PCR_rep_data_summary_", timestamp, ".csv")
...
# Create the file name with the timestamp (NOTE:Change the MiSeq number to the dataset you are working with)
file_name1 <- paste0("~/git/metabarcoding_workflow/results/MiSeq13_final_data_summary_filled_filter_sites_", timestamp, ".csv")
