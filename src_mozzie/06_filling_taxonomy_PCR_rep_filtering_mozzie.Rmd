...
final_df <- read.csv(here::here("results/final_data_summaries/MiSeq19_vsearch_data_summary_13_08_2024.csv")) # change the Miseq run number and time stamp
```
...
file_name <- paste0(here::here("results/final_data_summaries/MiseqX_vsearch_data_summary_filled_"), timestamp, ".csv")
...
metab_long <- final_data_filled %>%
  pivot_longer(cols = A22BF3373rep1:S22YPB224rep3, names_to = "sample", values_to = "value") %>%
...
final_data_filled_clean <- final_data_filled %>%
  mutate(across(A22BF3373rep1:S22YPB224rep3, ~ replace_na(.x, 0)))  # Replace NA with 0 in specified columns

# Step 2: Pivot longer
metab_long <- final_data_filled_clean %>%
  pivot_longer(
    cols = A22BF3373rep1:S22YPB224rep3,  # Specify the range or individual column names
    names_to = "sample",
    values_to = "value"
  ) %>%
...
file_name <- paste0(here::here("results/final_data_summaries/MiseqX_vsearch_summary_filled_filt_PCR rep_"), timestamp, ".csv")
...
# Create the file name with the timestamp (NOTE:Change the MiSeq number to the dataset you are working with)
file_name1 <- paste0(here::here("results/Miseq19_final_data_summary_", timestamp, ".csv"))
