...
sp_fil_metab_long_rem <- filtered_dataframe_top_sp %>% filter((species %in% counts$species[counts_sp$count == 1] & site %in% counts_sp$site[counts$count == 1]))
...
final_data_filled_clean <- final_data_filled %>%
  mutate(across(Cont1:S21WAY30960rep3, ~ replace_na(.x, 0)))  # Replace NA with 0 in specified columns

# Step 2: Pivot longer
metab_long <- final_data_filled_clean %>%
  pivot_longer(
    cols = Cont1:S21WAY30960rep3,  # Specify the range or individual column names
    names_to = "sample",
    values_to = "value"
  ) %>%
...
file_name <- paste0("~/git/metabarcoding_workflow_Miseq16/results/Miseq16_final_filt_filled_PCR_rep_data_summary2_", timestamp, ".csv")
