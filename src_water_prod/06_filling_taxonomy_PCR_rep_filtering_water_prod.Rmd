...
final_df <- read.csv(paste0(uomshare, "/final_data_summaries/", miseq_folder, "_vsearch_data_summary_30_08_2024.csv")) # change the Miseq run number and time stamp
```
...
controls <- wide_summarised_df %>% 
  select(kingdom, phylum, class, order, family, genus, species, max_p_identity, DrosX, MAR2T)
controls <- controls %>%
 filter(MAR2T != 0 | DrosX != 0)
