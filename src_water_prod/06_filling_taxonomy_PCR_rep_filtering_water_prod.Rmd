...
final_df <- read.csv(paste0(uomshare, "/final_data_summaries/", miseq_folder, "_vsearch_data_summary_05_09_2024.csv")) # change the Miseq run number and time stamp
```
...
if (nrow(final_df) == nrow(final_data_filled)) {
  cat("The data frames have the same number of rows. Proceed \n")
} else {
...
file_name <- paste0(uomshare, "/final_data_summaries/modeling_data/", miseq_folder, "_vsearch_data_summary_filled_filt_PCR_long_format_", timestamp, ".csv")
# Save the dataframe with the new file name
