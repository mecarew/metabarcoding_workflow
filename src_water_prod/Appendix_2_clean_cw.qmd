...
all_records_aq_only_in_priv_lib <- all_records_aq_not_in_tfill[all_records_aq_not_in_tfill$asv_code %in% 
                                                                 c(priv_lib_spp$asv_code,priv_lib_lt97$asv_code),] #132
...
# if priv_lib_spp similarity is greater than tfill$max_p_identity, replace tfill$max_p_identity
if(priv_lib_spp$geneious_similarity[i] > tfill$max_p_identity[tfill$asv_code == priv_lib_spp$asv_code[i]])
tfill$max_sim_bold[tfill$asv_code == priv_lib_spp$asv_code[i]] <- priv_lib_spp$geneious_similarity[i]
tfill$max_p_update[tfill$asv_code == priv_lib_spp$asv_code[i]] <- 1
}
...
tfill$max_p_identity[ti] > priv_lib_lt97$geneious_similarity[i]
...
# If geneious_similarity is higher than the current max_p_identity, replace it
if(priv_lib_lt97$geneious_similarity[i] > tfill$max_p_identity[ti]) 
  tfill$max_p_identity[ti] <- priv_lib_lt97$geneious_similarity[i]
}
# MEL: I'm not super comfortable about the above, given the untrackability of your max_p_identity values, 
#      for instance
priv_lib_lt97[7,] 
tfill[tfill$asv_code == priv_lib_lt97$asv_code[7],]
#   In this case, because max_p_identity (92) is greater than geneious_similarity (90.2), the above assumes
#   that tfill is right (identifying this to Austroperlidae), while geneious_similarity would only take it to Plecoptera
#   But could 92 be just one of your manual changes to max_p_identity?  
#   I am aware this is sweating the small stuff here. I suggest we don't worry about it. OK?

# 2. replace max_p_identity with max_sim_bold if the latter is higher.
...
# 3. replace max_p_identity with max_sim_bold if the latter is <97 and the former is >=97
sum(tfill$max_sim_bold < 97 & tfill$sim_vsearch >= 97, na.rm = TRUE)  # 598
for(i in 1:nrow(tfill)){
