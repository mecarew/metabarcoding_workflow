...
bact_records <- c(bact_bold_asvs,  c("f1af7c78e098ac3d8f195adadae2a33c",
                                     "9bff0ede8650baaa8cd7f68ed6dbc22a",
                                     "1ce6b94c99bc212e868810434d5a4981",
                                     "2e093f1b6f74ca96e03af70850c2add0")) # 494 records
...
bact_records <- bact_records[!duplicated(bact_records$asv_code),] #500
tfill_all <- tfill_all[!tfill_all$asv_code %in% bact_records$asv_code,]  #15,226
all_records <- all_records[!all_records$asv_code %in% bact_records$asv_code,] #22,048

# correctin ids to avoid loss to non_aq
all_records$family[all_records$asv_code == "fcff214b2c3fc71eeaf0f5ec12a781b5"] <- "Staphylinidae"
all_records$family[all_records$asv_code == "7c1246ff83f53bbc07f9122a83eb31d4"] <- "Elmidae"
...
# ttabs <- list(taxon_gen = taxon_gen, taxon_fam = taxon_fam)
# writexl::write_xlsx(ttabs,
#          "~/temp/taxonomy.xlsx")
# system("mv ~/temp/taxonomy.xlsx ~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_source_files")
...
tfill <- only_aqu_inverts(tfill_all, 
                          non_aqu_macro_invert_already_loaded = TRUE)$include #11,856 records
# Keep excluded records aside for recompilation below
tfill_nonaq <- only_aqu_inverts(tfill_all, 
                          non_aqu_macro_invert_already_loaded = TRUE)$exclude #3,370 records
# And exclude records identified to phylum (1 Nemertea has a bold match, but not necessarily aquatic)
tfill_nonaq <- rbind(tfill_nonaq, tfill[is.na(tfill$class) & is.na(tfill$species),]) #3,381
...
tfill_nonaq <- rbind(tfill_nonaq, tfill[!is.na(tfill$species) & tfill$species == "Arthropoda environmental sample",]) #3,383
tfill <- tfill[is.na(tfill$species) | tfill$species != "Arthropoda environmental sample",] #11,843

asvs_bin_uri <- unique(bold_results$asv_code[bold_results$similarity >= 97])  #20,588
asvs_bin_uri_lt97 <- unique(bold_results$asv_code[!bold_results$asv_code %in% asvs_bin_uri])  #1,499 
...
all_records_non_aq <- only_aqu_inverts(all_records, 
                          non_aqu_macro_invert_already_loaded = TRUE)$exclude #3,248 records
...
tfill_nonaq <- rbind(tfill_nonaq, 
                     tfill[tfill$asv_code %in% tfill_phylum_only_excl$asv_code,
                           match(names(tfill_nonaq),names(tfill))]) #3,393
...
tfill$species[!is.na(tfill$species) & tfill$species == "Notalina bifara"] <- "Notalina bifaria"
tfill$species[!is.na(tfill$species) & tfill$species == "Nososticta solitaris"] <- "Nososticta solitaria"
tfill$species[!is.na(tfill$species) & tfill$species == "Notalina bifara group B-ABV8023"] <- "Notalina sp. B-ABV8023"
  # See check below to confirm that Notalina sp. B-ABV8023 is different from Notalina bifaria
tfill$species[!is.na(tfill$species) & tfill$species == "Notalina sp. B-AAX5636"] <- "Notalina bifaria"
tfill$species[!is.na(tfill$species) & tfill$species == "Mirawara sp. AV1"] <- "Mirawara sp. B-AAV0800"
tfill$species[!is.na(tfill$species) & tfill$species == "Dicranomyia whitei whitei"] <- "Dicranomyia whitei"
tfill$species[!is.na(tfill$species) & tfill$species == "Haplotaxida sp. MC-4"] <- "Aulodrilus sp. MC-4"
```
...
for(i in 1:nrow(tfill)){
  if(tfill$asv_code[i] %in% names(match_list_bin_uri)){
    ...
    threshold <- 97
    # 3 records with very nearly 97% similarity we judge should be lumped
    if(tfill$asv_code[i] == "58c26bfd58265d2c2d50ebdf6eb1043c") threshold <- 96.98 # Austrosimulium, AEH7071 
    if(tfill$asv_code[i] == "3a6b0060a7a6f5d257a30a2ddd9622bf") threshold <- 96.94 # Micronecta, ACG1960 
    if(tfill$asv_code[i] == "d86c02bd1f826a07fb23c5d71811349c") threshold <- 96.96 # Dinotoperla, ACD5693
    matchi_97 <- matchi[matchi$similarity >= threshold,]
    if(nrow(matchi_97) > 0)
...
for(i in 1:nrow(tfill_spp_bins)){
    ...
    threshold <- 97
    # 3 records with very nearly 97% similarity we judge should be lumped
    if(tfill_spp_bins$asv_code[i] == "58c26bfd58265d2c2d50ebdf6eb1043c") threshold <- 96.98 # Austrosimulium, AEH7071 
    if(tfill_spp_bins$asv_code[i] == "3a6b0060a7a6f5d257a30a2ddd9622bf") threshold <- 96.94 # Micronecta, ACG1960 
    if(tfill_spp_bins$asv_code[i] == "d86c02bd1f826a07fb23c5d71811349c") threshold <- 96.96 # Dinotoperla, ACD5693
    bins <- matchi[matchi$similarity >= threshold,]
...
tfill <- tfill[!tfill$species %in% ambig_ids,] #19,862
...
"AGO0445" = "Austroargiolestes icteromelas", # BOLD has no consensus on which Austroargiolestes species this is, but calling this one icteromelas so that the code below calls them all Austroargiolestes icteromelas group
...
"ADC3729" = "Telephlebia brevicauda group", # BOLD calls it variously "Telephlebia brevicauda/godeffroyi" contains "Telephlebia godeffroyi", "Telephlebia sp_cyclops/tryoni", "Telephlebia brevicauda", "Telephlebia tryoni" 
...
"ABV8023" = "Notalina sp. B-ABV8023",
"ACE5915" = "Corbicula fluminalis", # so that the collective Corbicula bins get call Corbicula fluminalis group.
"ABW3343" = "Macrogyrus angustatus")
...
tfill_spp_bins <- tfill_spp_bins[!tfill_spp_bins$bin_uri %in% non_aqu_bins,] #29,062
...
tfill$species[tfill$species %in% c("Culex orbostiensis","Culex australicus")] <- "Culex australicus group" # Many other species names given to these sequences in BOLD
tfill$species[tfill$species %in% c("Nais stolci","Nais pardalis")] <- "Nais stolci/pardalis"
tfill$species[tfill$species %in% c("Kiefferulus cornishi","Kiefferulus intertinctus")] <- "Kiefferulus intertinctus" # only name given to these sequences in BOLD
...
tfill$species[tfill$species %in% c("Gynoplistia bimaculata","Gynoplistia lowanna")] <- "Gynoplistia bimaculata group" # BOLD records also contain the name "Gynoplistia biangri" 
...
sum(!is.na(tfill$family[tfill$max_p_identity < 95]))  #277 records with a family value but sim <95 (some also have a genus listed)
...
# Hydrobiosella waddama AAZ5125 and similarity 0.9707 to one record of AAZ4813...
# just over the criterion.  Sufficiently different to keep separate. Thus:
tfill_spp_bins <- tfill_spp_bins[!(tfill_spp_bins$species == "Hydrobiosella waddama" &
                                             tfill_spp_bins$similarity == 97.07),]
...
for(i in 1:nrow(tfill_asv_spp_unq)){
  species_sim <- 97
  if(tfill_asv_spp_unq$species[i] == "Hydrobiosella waddama") species_sim <- 97.08
   # problematic record of Hydrobiosella waddama explained at bottom of last chunk
  if(tfill_asv_spp_unq$species[i] == "Austrosimulium sp. B-AEH7071") species_sim <- 96.99
  if(tfill_asv_spp_unq$species[i] == "Dinotoperla thwaitesi") species_sim <- 96.97
  if(tfill_asv_spp_unq$species[i] == "Micronecta sp. MC-2") species_sim <- 96.95
  # ...and the three exceptions with slightly adjusted thresholds (by asv_code) when assigning bins above
  # Find all bin_uris associated with the current asv_code with an acceptable degree of similarity
...
# Confirm that Notalina sp. B-ABV8023 is different from Notalina bifara
# bold_results[bold_results$bin_uri == "ABV8023" & bold_results$similarity >= 97,] -> x
# unique(bold_results$bin_uri[bold_results$asv_code %in% unique(x$asv_code) & bold_results$similarity >= 97])
#  None of the records with bin_uri = "ABV8023" overlap with any other bin_uris, so it is different.
### Check these!
priv_lib_bs <- unique(priv_lib_spp$species[grep("B-",priv_lib_spp$species)])[right(unique(priv_lib_spp$species[grep("B-",priv_lib_spp$species)]),7) %in% tfill_spp_bins$bin_uri]
bs <- data.frame(tfill_spp = tfill$species[match(priv_lib_spp$asv_code[priv_lib_spp$species %in% priv_lib_bs], tfill$asv_code)],
           priv_spp = priv_lib_spp$species[priv_lib_spp$species %in% priv_lib_bs])
bs <- bs[bs$tfill_spp != bs$priv_spp,]
bs$correct_name <- NA
bs$bin_uri <- right(bs$priv_spp,7)
for(i in 1:nrow(bs)){
  bs$correct_name[i] <- unique(tfill_spp_bins$species[tfill_spp_bins$bin_uri == bs$bin_uri[i]])
}
bs[bs$priv_spp != bs$correct_name,]
# Just four names needing correcting:
corrections <- c("Ablabesmyia sp. B-AEN3809" = "Ablabesmyia sp. B-ACG8949 group",
                 "Apsectrotanypus sp. B-AEM8177" = "Apsectrotanypus sp. B-AEM8177 group",
                 "Atalophlebia sp. B-ABV8733" = "Atalophlebia sp. B-ABV8733 group",
                 "Austrosimulium sp. B-AEH7071/ABX5398" = "Austrosimulium sp. B-ABX5398 group",
                 "Notalina bifara group" = "Notalina sp. B-ABV8023")
corrections_tab <- data.frame(old_name = names(corrections), new_name = unname(corrections))
for(i in 1:length(corrections)){
  priv_lib_spp$species[priv_lib_spp$species == corrections_tab$old_name[i]] <- corrections_tab$new_name[i]
}
...
for(i in 1:nrow(priv_lib_spp)){
  ...
  two_bs <- (grepl("\ B-", tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]]) &
             grepl("\ B-", priv_lib_spp$species[i])) | 
                     tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]] == "Notalina sp. B-ABV8023"
    # corrected_species_name in private library incorrectly grouped these records as Notalina bifaria group
  # # If two_bs, trust the tfill process above, and leave the tfill name as it is
  ### Now this two_bs thing caused problems...try without out
  # if(!(two_bs)){
    ...
    if(!two_bs & priv_lib_spp$species[i] != tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]] &
       tfill$max_sim_bold[tfill$asv_code == priv_lib_spp$asv_code[i]] >= 97) stop("1")
    }
 # if(!two_bs){
    if(tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]] %in% unique(tfill_spp_bins$species) & 
       tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]] != priv_lib_spp$species[i] & 
       !priv_lib_spp$species[i] %in% c("Austropeplea sp. B-GBAAZ18142-24",
                                       "Contacyphon lindensis",
                                       "Trombidioidea sp. MC-1"))  
      # Accept these correct changes from priv_lib (stop if any other disagreements)
       stop("2")
      # Don't want to change the name if it's already in tfill_spp_bins
    tfill$species[tfill$asv_code == priv_lib_spp$asv_code[i]] <- priv_lib_spp$species[i]
 #   }
if(is.na(tfill$max_sim_bold[tfill$asv_code == priv_lib_spp$asv_code[i]]))
...
# Check tfill[694,]
# A resulting genus correction:
tfill$genus[grep("Contacyphon",tfill$species)] <- "Contacyphon"
...
for(i in 1:nrow(priv_lib_lt97)){
  if(priv_lib_lt97$asv_code[i] %in% tfill$asv_code){
...
# if priv_lib_lt97 geneious_similarity is greater than tfill$max_sim_bold, 
...
sum(tfill$max_sim_bold > tfill$sim_vsearch, na.rm = TRUE) #12048
for(i in 1:nrow(tfill)){
...
sum(tfill$max_sim_bold < 97 & tfill$sim_vsearch >= 97, na.rm = TRUE)  # 302
for(i in 1:nrow(tfill)){
...
tfill$dubious_id <- 0
tfill$dubious_id[tfill$max_p_identity <= 86.2] <- 1 # sum(tfill$dubious_id) # MEL: 25 records to consider removing from tfill

# After inclusion of some priv_lib entries to genus/family
tfill$family[tfill$species_old == "Austrocyphon ovensensis"] <- "Scirtidae"
...
for(i in which(tfill$asv_code %in% priv_lib_spp$asv_code)){
  if(grepl("Haplotaxida",tfill$species[i]) | grepl("Enchytraeida",tfill$species[i]) |
    ...
    if(is.na(tfill$species[i])){
      # 9 priv_lib records without species names. A check to make sure that doesn't matter
      if(is.na(priv_lib_spp$species[priv_lib_spp$asv_code == tfill$asv_code[i]])) stop("1")
     tfill$species[i] <- priv_lib_spp$species[priv_lib_spp$asv_code == tfill$asv_code[i]]
    }
...
thresholds <- rep(97,nrow(tfill))
# See above for adjusted thresholds
thresholds[match(c("58c26bfd58265d2c2d50ebdf6eb1043c","3a6b0060a7a6f5d257a30a2ddd9622bf",
                 "d86c02bd1f826a07fb23c5d71811349c"), tfill$asv_code)] <- c(96.99,96.95,96.97)
tfill$species[tfill$max_p_identity < thresholds] <- NA
gen_levels <- tfill[tfill$max_p_identity < thresholds & tfill$max_p_identity >=95,
...
check1 # 22 records checked and agreed that script changes are correct
# tfill$max_p_identity[tfill$asv_code %in% check1$asv_code]
check2 # 79 records checked and agreed that script changes are correct
# Disagreeing BOLD names have <97 match
...
tfill$family[!is.na(tfill$order) & tfill$order == "Salticidae"] <- "Salticidae"
tfill$order[!is.na(tfill$family) & tfill$family == "Salticidae"] <- "Araneae"
...
to_exclude <- only_aqu_inverts(tfill,
                          non_aqu_macro_invert_already_loaded = TRUE)$exclude # 25
tfill_nonaq <- rbind(tfill_nonaq,tfill[tfill$asv_code %in% to_exclude$asv_code,
                                       match(names(tfill_nonaq),names(tfill))]) #3543
tfill <- tfill[!tfill$asv_code %in% to_exclude$asv_code,] #19737
...
for(i in 1:nrow(fam_levels)){
  ...
  if(nrow(fam_match) == 0){  # if there are no fam_matches in bold, save records in check3
    ...
    if(nrow(fam_match) > 1){  
      # if bold lists more than 1 family name, and if that name is by far the most common, select that one
    ...
    if(is.na(fam_levels$family[i]) | (!is.na(fam_levels$family[i]) & fam_match$family != fam_levels$family[i]))
      # if tfill family entry doesn't match bold, save records in check4
...
check3 # 9 cases where there was a family name in tfill, but BOLD didn't have one
# tfill$max_p_identity[tfill$asv_code %in% check3$asv_code] # these should not have families: corrected below
check4[!is.na(check4$family),] # 5 mismatches with family names (all of these should have no family match)
range(tfill$max_p_identity[tfill$asv_code %in% check4$asv_code])
# all of which look highly uncertain in bold records: mark as dubious_id = 2
tfill$dubious_id[tfill$asv_code %in% check4$asv_code[!is.na(check4$family)]] <- 2
# Actually the last one looks ok
tfill$dubious_id[tfill$asv_code == "d79729091c4bbfd0245a0e878424248b"] <- 0

tfill$kingdom[tfill$phylum == "Arthropoda"] <- "Eukaryota"
...
# And final check/correction of taxonomic hierarchy for all named species
for(i in 1:nrow(tfill)){
  if(!is.na(tfill$species[i])){
    first_word <- strsplit(tfill$species[i]," ")[[1]][1]
    if(first_word %in% taxon_gen$genus){
      tfill$genus[i] <- first_word
      if(is.na(tfill$family[i])) {
        tfill$family[i] <- 
        taxon_fam$family[taxon_fam$famcode == 
                    taxon_gen$famcode[taxon_gen$genus == first_word]]
      }
      if(is.na(tfill$order[i])) {
        if(tfill$family[i] != "Aeolosomatidae") {  # order of Aeolosomatidae incertae sedis
        tfill_orderi <- aggregate(tfill$order[tfill$family == tfill$family[i]],
                            by = list(order = tfill$order[tfill$family == tfill$family[i]]),
                            FUN = length)
        tfill$order[i] <- ifelse(tfill$family[i] == "Phreodrilidae",
                                 "Haplotaxidae",
                              tfill_orderi$order[which.max(tfill_orderi$x)])
        }
      }
    }
  if(first_word %in% c(taxon_fam$family, subfams$subfamily)){
      tfill$genus[i] <- NA
      if(is.na(tfill$family[i])) {
        fami <- ifelse(first_word %in% taxon_fam$family, first_word, 
                       subfams$family[subfams$subfamily == first_word])
        tfill$family[i] <- fami
      }
      if(is.na(tfill$order[i])) {
        if(tfill$family[i] != "Aeolosomatidae") {  # order of Aeolosomatidae incertae sedis
...
# replace subfamily names with family names
...
# Incorrect order discovered on final checks
tfill$order[!is.na(tfill$genus) & tfill$genus == "Fluviolanatus"] <- "Venerida"
...
wd <- "~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_library/"  # data/
