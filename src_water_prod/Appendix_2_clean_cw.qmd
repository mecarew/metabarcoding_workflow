...
#MEL: this is a temporary measure to let you decide if you want to include some or all of the 126 unmatched genbank/privlib records
#     I created this table in Appendix_2_cross_check_manual.qmd from the final 126 of check_no_bold (L65, now commented out)
unmatched_priv_lib <- read.csv("~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_source_files/unmatched_priv_lib.csv")
# if you want to include the species name for only some of these, here are some suggestions:
# # If you want to include only a subset, I suggest you subset this table by name as a first preference (with commented out justification)
# # as an example...
# unique(unmatched_priv_lib$checked_species_name)
# unmatched_priv_lib <- unmatched_priv_lib[unmatched_priv_lib$checked_species_name %in% c("Austroneurorthus bruneipennis", "Leptoperla kallistae")]
# # or as a second, less desirable, preference, you could filter by asv_code, but if you do, please document your reasoning for inclusion/exclusion of each asv_code
# With unmatched_priv_lib containing all of the species-level ids that you want to retain, then you just need to uncomment
#  L1176-1181, and L1208-1212 (including the closing brace on L1208), and re-run the full script. 
# Your selected unmatched priv_lib records should now be in the final tfill table 
# If you don't change unmatched_priv_lib above, then all 126 records will be included with checked_species_names
...
tfill$species[!is.na(tfill$species) & tfill$species == "Notalina bifara group B-ABV8023"] <- "Notalina sp. B-ABV8023"
  # See check below to confirm that Notalina sp. B-ABV8023 is different from Notalina bifara
tfill$species[!is.na(tfill$species) & tfill$species == "Notalina sp. B-AAX5636"] <- "Notalina bifara"
...
# MEL: on final+5 check, I decided that max_sim_bold should not be updated if it is a B- species
#      This is because of the inconsistency arising from asv_code "8ca4a080d6a6bf02b39c7ad1e83b0439"
#      Your priv lib calls it Apsectrotanypus sp. B-AEM8177 with a genious_similarity of 99.5
#      But the bold results say that the best match to AEM8177 (or any bin) is 96.73. 
#      I thus decided that in such cases, max_sim_bold should remain what bold says.  
#      (Otherwise, this species ends up as its original name: Apsectrotanypus sp. B-AEM8177/AEM8936, also wrong,
#       rather than the correct outcome of only identifying it to Genus.)  Am I right, or have I got the wrong end of the stick?
if(is.na(tfill$max_sim_bold[tfill$asv_code == priv_lib_spp$asv_code[i]]))
...
# # Code to exclude any change to max_p_identity or to species name 
#   # for the species in unmatched_priv_lib, loaded in first chunk of script
#   # Commented out for now, because CW doesn't think it's the right thing to do
#   # But here so that MEL can choose to keep the checked species names
#   # But if so, MEL, please provide commented out justification!!!
#   if(!priv_lib_lt97$asv_code[i] %in% unmatched_priv_lib$asv_code){
...
# } 
# # Add in unmatched_priv_lib
# for(i in 1:nrow(unmatched_priv_lib)){
#   tfill$species[tfill$asv_code == unmatched_priv_lib$asv_code[i]] <- unmatched_priv_lib$checked_species_name[i]
# }
...
tfill[tfill$asv_code %in% "8ca4a080d6a6bf02b39c7ad1e83b0439",]
