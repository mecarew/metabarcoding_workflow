...
[MEL#3: I've now made sure that the above are correct as you specify, except for the last one. You might want to look at the final split of Zavrelimyia species names. I struck fewer problems with overlapping bin_uris for Zavrelimyia than in the original Appendix 2 script.  I *think* it's right, but you should check!...you can delete all of the above from CHRIS#2 down if you are happy with that!] CHRIS#3: Happy with that but I let you delete.
...
# [MEL#3: see my email about these three problems.  If you are happy with the names that this script gives these three records (as given in the email), then you can delete this set of comments and nothing else is necessary.  If you want any of the names changed, then additional script lines are necessary. Let me know.] CHRIS#3: Addressed in e-mail
...
# [MEL#3: This is resolved below. Check you are happy...]

#[MEL#3: I've deleted a chunk of comments here about max_p_update problems to keep things simple. I now remember what I meant by some of the recomended_name values in your table not looking right.  They were 7 full species names (rather than a higher taxonomic level) that I couldn't verify from BOLD, and the script just returned higher order unidentified names, incorrectly I think. The code below returns the asv_codes for the seven of them (as an object called x, from your original max_p_update table loaded above as mp1), and I have given them all tentative species names for you to check.
# 1.  "Illiesoperla mayi". I agree that with your assessment of the Illiesoperla mayi record. Using https://v4.boldsystems.org/index.php/IDS_OpenIdEngine, I get a match with Illiesoperla mayi at 96.97%, but I get no match using the bold::bold_identify() function in R (strange!). Assuming you're happy with a 96.97% match, I've added a correction below (with note that we are accepting 96.97% as a near enough match). But I have left a "JUSTIFICATION" comment for you to add a justification for this bending of the rule. 
# 2. "Sphaerium sp. B-ADJ8477". I don't think we should give it this name, as it is only 96.74% similar to that bin_uri.  Feel free to give it that value in the code below (change "Sphaerium sp. (Unident.)" to "Sphaerium sp. B-ADJ8477" or an MC code) if you disagree (and add a justification if you do disagree).
# 3. "Hydra vulgaris (sp. Unident.)"  Same story as for Illiesoperla, except the similarity is less.  I've assumed Hydra vulgaris is ok and have left a "JUSTIFICATION" comment for you to add a justification for this bending of the rule. If you want to just call it Hydra sp. (Unident.), then just change the code as I suggest for 2.
# 4. "Nousia sp. B-ABV9190". Same story as 2.
# 5. "Coloburiscoides giganteus" See comments in code. I think it is too dissimilar to be giving it this species name. (but  change as above and add justification if you disagree)
# 6. "Nousia sp. B-AAV4108"
# 7. "Cernosvitoviella aggtelekiensis"  Only 95.45% similarity with this species (using both BOLD methods). I have assumed unidentified genus would be better...or you could give it an MC code if you prefer.] 

#CHRIS#3: In response to above. If they don't match above the 97 threshold but greater than 95 it is best to identify to genus. However I think some on the discrepancies here between us may be coming from the version of the identification engine we are using, as the libraries for each version are different. As mention above, I am using the latest one https://id.boldsystems.org/ and searching the public and private animal database.
...
# [MEL#3: Just a note that I have updated the exclusion table and the taxon_gen table to include/exclude
#  the problematic taxa that you helped me with in the last revision. Thanks!] #CHRIS#3: Thanks
...
# [MEL#3: updated stats ...if you are happy, this can be deleted.the above loop corrected:  
# sum(tfill$family_recode) # = 3066 family names. To inspect the unique changes, 
#   unique(tfill[tfill$family_recode == 1,c("family","family_old"),]) # Just 18 unique family changes
...
# [MEL: dodge_bs lists 55 species names that you created using bin_uris that either have no matches in the bold database (17: sum(dodge_bs$some_bin_matches == 0) ), or some records given that name have no match (36: sum(dodge_bs$some_bin_matches) )].  What do you want to do about them? I will leave them as they are for now.] CHRIS: I think ones with no matches are new BINs not yet updated into the database. For example, if I search for BIN:AEW4330 on the BIN database there is no match. If I put the sequence in the 'Barcode ID' and search it matches (100) to Chaetogaster with bin AEW4330. This was the case for all the ones I checked. So leave these BINs for now.
# [MEL#2: fair enough. Probably worth including an explanatory section here justifying that. Could you have a crack at writing something for a general audience to understand this decision] CHRIS#2: For some asv records we found no matches in the bold database or some records given that name have no match using the API. However, when these asv records were identified using the BarcodeID web interface additional bold matches and bin_uri's were found.
# [MEL#3.  As long as you are sure about this.  I checked the first on in dodge_bs by submitting the first of the 4 sequences returned by this line
unique(tfill$asv_sequence[tfill$species == dodge_bs$species[1]])
# It's a 100% hit with Chaetogaster sp. CEJ, but I couldn't work out how to see a bin_uri to check your name of Chaetogaster sp. B-AEW4330.
# Could you write some words on how you found the bin_uri?]
#CHIRS#3: 'For some sequences the bin_url was only available via the BarcodeID engine. The bin_url was recorded for these sequences from  'combined hits' table under the 'PID (BIN)' heading.'I am confident theses bin_urls are correct. You need to use the new Id engine with animal private and public library  https://id.boldsystems.org/. The V4 does not give BINs. 
...
#[MEL#3: the following commented out lines might be useful for checking changes that this script has made to your original namings.]
# sum(tfill$species != tfill$species_old)  # 1952 records have had their species name changed
# changed_species <- unique(tfill[tfill$species != tfill$species_old, c("species","species_old")])
# nrow(changed_species)  #238 species names changed.
# sum(grepl("Zavrelimyia", changed_species$species)) # including 13 distinct Zavrelimyia groups. More than before?
# CHRIS#3 for the 'changed_species' names Hydraena sNA should be Hydraena sp. B-AFJ4718. I could not see any other issues. Zavrelimyia is a nightmare with there being a few distinct species (bins) but most others formed groups. I can't see issues with your groupings.
