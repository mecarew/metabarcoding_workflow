...
# [MEL: I don't really get your explanation of what max_p_update is for...can you rephrase? CHRIS: The max-p-update adds taxonomic information and corrects max-p-identity for asv's resulting from errors in the vsearch reference library (which happens when you do mass data dumps). It also adds taxonomic identifications and max-p-identity for species not in the vsearch reference library (i.e., specimens sequenced after the vsearch library was assembled)]
...
- additions_asv_library_oct24.xlsx (additional records for truncated_fills); [MEL: can you describe where these additional records come from? CHRIS: These are records found after the 'asv_library_97_aug24.xlsx' was constructed]
...
# [MEL: see email about what I have assumed needed doing here and correct as necessary CHRIS: I have responded to this in an e-mail. Sorry]
...
# [MEL: species names for those records are updated below - I have not used the 'recomended_species_name' field: see my email CHRIS: I have seen your e-mail and was unclear what the issue was?)
...
# [MEL: as per my email, is this the right thing to do? CHRIS: remove duplicates from the truncated_fills_df]
...
#  use NA for missing values....let me know if it is a problem CHRIS: I don't think it will be an issue]
...
#[MEL: this is a weird non-aquatic entry that you corrected family and genus in two different places in your script. CHRIS: This would just be a duplication of the code. I think. It was likely something that identified as an invertebrate but was not)
# Here I also correct the other taxonomic levels, but I really don't want to be messing around with non-aquatic species.
# Are you sure this is right.  IT IS VERY ODD! CHIRS: This is how BOLD identifies this asv] 
...
#[MEL: in your Appendix 2 script, you deleted these three records. I haven't done so here - they look alright to me. CHRIS: there was something odd with these records so I revert to the taxonomic identifications given by vsearch (as I only produced the asv tables to correct errors). These records no species match on BOLD.]
...
# [MEL: code for deleting them if you are sure they really should be deleted. CHRIS: I think that they should be deleted. This means they will appear in the miseq data as asv without a species identification]
...
#[MEL: are you also happy to leave Isopoda sp. MC-1 out of aquatic consideration? CHRIS: It ambiguous as to whether it is aquatic or not so it can be omitted]
...
# [MEL: Perhaps you want to call this "Notalina bifara group"? (the two bin_uris overlap...you had originally called this "Notalina bifara  B-ABV8023 group"!) CHRIS: That's fine.]
tfill$species[tfill$species == "Ferrissia sp. MC-1"] <- "Ferrissia sp. B-AAE6642"
# [MEL: You had renamed this "Ferrissia sp. B-AAY1880" group" but it only matches "ADR7371", "AAE6642" and "ADR3550". There is no AAY1880 in the library dataset. Are you happy for this to be called "Ferrissia sp. B-AAE6642 group"?) CHRIS: Yes.]
...
#  as the above commented out line shows that they are all already there CHRIS: Ok]
```
...
#   match_list_bin_uri[names(match_list_bin_uri) %in% naid_asvs]] CHRIS: Yes
...
#       records that have the same bin_uris (some of which were missed by your code). Is my logic sound? CHRIS: Yes. I am happy with what you have done here.
...
# The following is my solution to the problem of Chamaedrilus and Cognettia using bin_uris.  Do you agree? CHIRS: That sounds logical to me]
...
# I will make these tables accessible in the final version...] CHRIS: OK. but there may be some inconsistency between these names and those on BOLD/GenBank. 
...
# [MEL: The following assumes that the "class","order","family","genus" details  are correct. Is that right? CHRIS: Yes. This is right. I can't quite make sense of the bit below. Sorry. Which data should I be looking at?
...
# [MEL: apart from the reservations above, this takes care of the 46 records in your asv_code script that were set to "". See "unravel_asv_code_changes.R"] CHRIS: OK
...
# [MEL: To get the following loop to run, I needed to exclude a few taxon names.
 ...
 #      "Eupodidae"(exclude) ,"Triophtydeidae" (exclude),"Isopoda sp. MC-1" (excluded),"Listronotus" (not sure so maybe safe to exclude),"Paederus"(excluded),"Spanioda"(excluded),"Geranomyia" (aquatic - Limoniidae),"Marionina" (some aquatic species but not all so I guess exclude if id only at genus)
...
# [MEL: Do you have reservations about me using the mwbugs taxonomy to correct the genus and family names in the 
 #      the asv library? See below for some diagnostics to see what I have changed]  CHRIS: I don't think so at this stage
for(i in 1:nrow(tfill)){
  recordi <- tfill[i,]
...
#      (I have assumed that your naming of "Ozeoura convoluta" and "Atrichopogon" are correct)] CHRIS: Ozeoura convoluta would one of Zac's ids and Atrichopogon was a genus id for a specimen originally at family level
