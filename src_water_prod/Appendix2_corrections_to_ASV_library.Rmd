...
[MEL#2: You didn't respond to the above. I was hoping you would find this a useful explanation of the logic behind my changes, and a guide for making similar corrections in the future.  If you are happy with that (and happy with my logic, please feel free to delete these two comments)]
...
# [MEL: As you noted "Riethia zeylandica is not found in Australia": the above is a temporary name to be grouped below] [MEL#2: No response - if you happy, then please delete these two comments]
...
# [MEL: Perhaps you want to call this "Notalina bifara group"? (the two bin_uris overlap...you had originally called this "Notalina bifara  B-ABV8023 group"!) CHRIS: That's fine.] [MEL#2: Note that the code calls it Notalina bifara - are you happy with that, or do you want it called "Notalina bifara group"?]
...
[MEL2: Let's leave these comments here for now until the final version, and I can have a final go]

The chunk is set not to evaluate, as it took ~30 h to run.

[MEL: Don't run this chunk!] [MEL#2: Just another reminder in case you are running through the code again...can be deleted when we are happy]
...
[MEL: Don't run this chunk either!] [MEL#2: Just another reminder in case you are running through the code again...can be deleted when we are happy]
...
[MEL#2: As above - It would be good if you included some short explanatory text here to explain the logic of what's going on here; I can't help but think there must be a more transparent way to have made these changes...but let's just go with it. Please delete these two comments when you've added some text.]
```{r}
...
# [MEL#2: Thanks Mel, that's all useful. I will update the exclusion table accordingly - not important for this process, so I'll do it when I get a bit more time (soon) - As for the Enchytraeids - should I exclude species that you don't list above?]
...
# [MEL#2: just checking, given your confusion about running lines above, have you checked the outputs of the commented out  lines above?..If you are still happy, then please delete this group of comments]
```
...
# [MEL#2: fair enough. Probably worth including an explanatory section here justifying that. Could you have a crack at writing something for a general audience to understand this decision]
...
[MEL#2: that's all good, but once again it would be good to include text here explaining these decisions for the reader]
