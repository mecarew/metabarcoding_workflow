...
# Check requisite taxonomy, exclusion, and BOLD files are in data directory, 
...
During the preparation of this file, BOLD launched a new version of their database (https://id.boldsystems.org/), which would require substantially revised R code to replicate the processes below.  We subsequently discovered some discrepancies between the two databases, which are noted and corrected below.

The following chunk is step 1, saving the full list of BOLD matches (each with name = asv_code) to "~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_library_97_match_list_to_BOLD_2024-12-05.rda" (i.e. a unimelb network drive). This file, and the one created by the subsequent chunk have then been saved to an OSF repository (https://osf.io/hvx69/). The script above ensures these files have been saved to the user's local data directory, from which they are read in below.  

The chunk is set not to evaluate, as it took ~30 h to run. (It is unnecessary to do so, as its resulting file is read in below)
...
The list was saved as "~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_library_97_match_list_with_bin_uris2024-12-05.rda". (See above for who this was managed)

As above, the chunk is set not to evaluate, as it also took ~30 h to run. (It is unnecessary to do so, as its resulting file is read in below)
...
load("data/asv_library_97_match_list_to_BOLD_2024-12-15.rda")
...
# None of these three had a matching bold record for me to assess their veracity. However, matches were found using the Barcode ID engine https://id.boldsystems.org/]
...
tfill$cw_checked <- 1
# Final ad-hoc corrections to family field
