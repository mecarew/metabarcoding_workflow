...
#***NOTE: YOU WILL NEED TO CHANGE THE MISEQ FOLDER NAME to the directory you are working in and the directory to your own home drive if you have not started at the workbook 00***
...
```{r}
primers_filename <- here::here("ref/primers.tsv")
suppressMessages(
...
```{r}
metadata_filename <- here::here("ref/metadata.tsv")
suppressMessages(
...
data.frame(sample_name=samples_df$sample_name,
           r1_exists=file.exists(here::here(samples_df$filename_r1)),
           r2_exists=file.exists(here::here(samples_df$filename_r2))) %>%
  paged_table()
```
...
for (i in seq_len(nrow(samples_df))) {
  ...
  if (!file.exists(file.path(lib_dir, "forward.fastq.gz"))) {
    file.copy(from = file.path(here::here(x$filename_r1)), 
              to = file.path(lib_dir, "forward.fastq.gz"))
  }
  
  if (!file.exists(file.path(lib_dir, "reverse.fastq.gz"))) {
    file.copy(from = file.path(here::here(x$filename_r2)), 
...
```{r}
# If there::here are already qza files generated, don't list them here::here
dir_tree(here::here(import_data_dir), regexp="*qza", invert=TRUE)
```
...
# if you get a '2>&1' had status warning you will need to re-run this chunk
```
:::
...
# if you get a 2>&1' had status 1 warning you will need to re-run this chunk
```
...
```{r}
dir.create(paste0(uomshare, "/cache"))
