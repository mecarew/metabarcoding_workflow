...
# MEL: What do you want to do with these 187 priv_lib_lt97 records? Remove from the library altogether, or leave them in with only identification to the appropriate taxonomic level? If the latter, Do you want their max_p_identity values changed to geneious_similarity?
...
no_check_no_bold
# MEL: Only 8 now (Down from 11). The first seven being given no species name (as agreed last time), and are you happy calling the last Trombidioidea sp. MC-1?

# MEL: I think all of the following checks of check_with_bold look good. No further action required
...
nrow(x)
x
# reduced to 14, # MEL: I think I have corrected the few that you were concerned with now.
```
...
#      Perhaps I have mis-translated your code?
#      I've left your comments below, but isn't the critical question for these 3 species "how similar are your
#      private library sequences to the matching sequences in tfill?
#      e.g. for Diplodontus haliki how similar is 
tfill$asv_sequence[tfill$asv_code == "dc2aaa4fad40cb11a0f1c638eb455a86"]
#      to
       species_on_bold$DNA_seq[1]
#  If they are not adequately similar, I don't think we should be calling dc2aaa4fad40cb11a0f1c638eb455a86 Diplodontus haliki
...
# 3 (CW gets 2)
# The only one I can not justify is Tasmanocoenis sp. B-ACG1634 (Tasmanocoenis tillyardi/riekis (pecimen Co18Caen5). 
...
genei_results$priv_lib_species <- priv_bar_lib$priv_lib_species[match(genei_results$id, priv_bar_lib$id)]   # 2,710 records, right?
# MEL: note there are 141 records in genei_results without a match in priv_bar_lib: Is this expected?
...
genei_sp_results <- genei_results[genei_results$geneious_similarity >= 97,] # 1187 (was 1517 in your original notes, MEL) records in the no_bold_species_old match the private library. 
...
##      Changing that to species finds 38 records (some corrected, see below, so the number is smaller now: see below)
...
# 5 records with species = "Empididae sp. MC-BC2", priv_lib_species = "Empididae sp. MC-Co1"
   # I did not change these - so these two names must have come from you. Which one do you want?
...
tfill_name_diff[tfill_name_diff$asv_code %in% names(match_list_bin_uri),]
   # I think my renaming of these is right. And I presume you DO want to change max_p_identity for these to max_bold_sim?
...
# 22 records with species = "Notalina bifara group" and priv_lib_species = "Notalina sp. B-ABV8023"
   # As these do not match any bold records, I don't think the priv_lib_species is appropriate
   ...
   # If you are confident that these really are Notalina, then I think Notalina bifara group is a more appropriate name 
...
# 2 records with species = "Austrochiltonia australis" and priv_lib_species = "Austrochiltonia MC-1"
   ...
   # I am inclined to leave these as "Austrochiltonia australis"
#  1 Trombidioidea sp. B-AEI6567 and 3 Oecetis pechana were original names in the source data.  I have changed them to your 
   # priv_lib_species names, and they should no longer appear in this 
...
gb_asv_matches3 <- gb_search_results %>%
  filter(match >= 80, length > 200) %>% select(asv_code, match) %>%  #MEL: you had match < 80 here. Did you mean that???? 
...
# 144 record not on bold to check agaist genbank - CW gets 858...so does the genbank check need to be re-done??
...
### MEL: so what happens to these 183?  Should they be removed from the library????
check_no_bold_in_priv <- check_no_bold_in_priv[check_no_bold_in_priv$geneious_match >= 97,] # I get 41 you got 24
...
# MEL: What do you want to do with these 11 records?
```
