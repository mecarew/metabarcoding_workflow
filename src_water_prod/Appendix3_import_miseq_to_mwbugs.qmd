...
#[MEL: note that this excludes three samples with smpcodes starting with 337 (the 46 site study?) - 
#      There are no matching samples (that I can find) in your and Gen's spreadsheets that I have 
#     used to assemble these tables.
#      should they be in here? Are they composite samples like all the others?  More information needed
#CHRIS: These are re-runs from our initial experiment on the 10 sites used to determine how much sample to process for the sites covered in miseq runs 15-20. The remaining files are in run miseq13. I created a modified version of notebook 6 to process them as the experimental design was different. 
# ]
...
#CHRIS: Agreed. Delete Unionidae, Arcidae (checking these on GenBank both actually match a gastropod at ~92%). All the species you list below can not be confirmed as aquatic, so I agree with excluding them. 
...
```{r}
sequences <- unique(bugdata_spp[c("asv_code","asv_seq")])

biota_wrkng <- bugdata_spp[c("sample","asv_code","species")]
...
biota_wrkng <- bugdata_gen[c("sample","asv_code","genus")]
biota_wrkng$extraction_name <- substr(biota_wrkng$sample,1,nchar(biota_wrkng$sample)-4)
biota_wrkng$smpcode <- samples_miseq$smpcode[match(biota_wrkng$extraction_name, 
                                                   samples_miseq$extraction_name)]
biota_wrkng <- biota_wrkng[!is.na(biota_wrkng$smpcode),]
...
biota_wrkng <- bugdata_fam[c("sample","asv_code","family")]
biota_wrkng$extraction_name <- substr(biota_wrkng$sample,1,nchar(biota_wrkng$sample)-4)
biota_wrkng$smpcode <- samples_miseq$smpcode[match(biota_wrkng$extraction_name, 
                                                   samples_miseq$extraction_name)]
biota_wrkng <- biota_wrkng[!is.na(biota_wrkng$smpcode),]
...
#[MEL the 'count' field in new_biota = sum of records among the three (sometimes 2) miseq replicates. Is this sensible? Useful? Note that unidentified genera and families can then have a count > 3 if there are more than one unidentified records for a genus or family.  Would total mean number of reads per replicated be a more useful count value, or should we just use the value 1?] 
#CHRIS: What your counts likely suggest is there are more individuals where the count is higher but this may not always work. Some species may have high genetic diversity whereas others may this will also influence the number of asv associated with each species. While interesting I'd suggest we use presence/absence (value of 1) for the species data.

sample_asv <- rbind(sample_asv, unique(biota_wrkng[c("asv_code","smpcode")]))
bugcode_asv <- rbind(bugcode_asv, unique(biota_wrkng[c("asv_code","bugcode")]))

#[MEL: sorry to say at the last hurdle, bugcode_asv revealed e asv_codes that have been asigned to two different species: the following returns the probelmatic 3 asv_codes. I'm pretty sure this error is not my doing. The code below (separated by #####) does some digging that might help you find the problems...]
#CHRIS: the assigment for 2007d5c9d93583786864fb043f3f09c2 should be Russobex cuneatus and for c684b08eca0dc60e274029cabda6cd4c Nousia is correct. The issue is that I inadvertently used the old vsearch database with errors and will need to re-run the vsearch step.

#####
x_spp <- bugdata_spp[bugdata_spp$asv_code %in% unique(x$asv_code), c("sample","asv_code","species")]
x_spp$miseq_run <- miseq_samples$miseq_run[match(x_spp$sample, miseq_samples$seq_code)]
x <- bugcode_asv[duplicated(bugcode_asv$asv_code) | duplicated(bugcode_asv$asv_code, fromLast = TRUE),]
x_spp_agg <- aggregate(x_spp$species, 
                   by = list(asv_code = x_spp$asv_code, species = x_spp$species, miseq_run = x_spp$miseq_run), 
                   FUN = length)
x_gen <- bugdata_gen[bugdata_gen$asv_code %in% unique(x$asv_code), c("sample","asv_code","genus")]
x_gen$miseq_run <- miseq_samples$miseq_run[match(x_gen$sample, miseq_samples$seq_code)]
x_gen_agg <- aggregate(x_gen$genus, 
                   by = list(asv_code = x_gen$asv_code, genus = x_gen$genus, miseq_run = x_gen$miseq_run), 
                   FUN = length)
x_spp_agg # 2007d5c9d93583786864fb043f3f09c2 identified as different species in miseq runs 15 (Russobex cuneatus) and 18 (Psychoglypha subborealis) 
x_gen_agg # c684b08eca0dc60e274029cabda6cd4c identified as different genus in miseq runs 17 (Nousia) and 18 (Labiobaetis)

x_fam <- bugdata_fam[bugdata_fam$asv_code == "65f10fe16b1f902c065c0fd9edb0f9bf",]
miseq_samples$miseq_run[miseq_samples$seq_code == x_fam$sample[1]]
# and 65f10fe16b1f902c065c0fd9edb0f9bf identified as Polypedilum in miseq 18, but Chironomidae in miseq 15

# As these errors are at the family level, it is possible that similar errors have been missed by having begun this compilation by excluding non-aquatic taxa.  Here I do a quick check of the data without that exclusion:
for(i in 1:length(miseq_dirs)){
  miseq_path <- paste0("~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/miseq", miseq_dirs[i],"/")
  bugdata_file <- paste0("miseq", miseq_dirs[i], "_final_data_long_format_18_02_2025.csv")
  bugdata_i <- read.csv(paste0(miseq_path, "metabarcoding_workflow/final_modeling_data/", bugdata_file))
  bugdata_i <- bugdata_i[c("sample","asv_code","family","genus","species")]
  bugdata_i$miseq_run <- miseq_dirs[1]
  if(i == 1){bugdata <- bugdata_i}else{bugdata <- rbind(bugdata, bugdata_i)}
}
bugcode_asv_all <- unique(bugdata[c("asv_code","family","genus","species")])
dup_asvs <- bugcode_asv_all[duplicated(bugcode_asv_all$asv_code) | 
                              duplicated(bugcode_asv_all$asv_code, fromLast = TRUE),]
dup_asvs <- dup_asvs[order(dup_asvs$asv_code),]
dup_asvs
x_all <- bugdata[bugdata$asv_code %in% unique(dup_asvs$asv_code), c("sample","asv_code","family","genus","species")]
x_all$miseq_run <- miseq_samples$miseq_run[match(x_all$sample, miseq_samples$seq_code)]
x_all_agg <- aggregate(x_all$asv_code, 
                   by = list(asv_code = x_all$asv_code, family = x_all$family, 
                             genus = x_all$genus, species = x_all$species, 
                             miseq_run = x_all$miseq_run), 
                   FUN = length)
x_all_agg <- x_all_agg[order(x_all_agg$asv_code),]
x_all_agg
# [MEL: I think it's worth having a look through these 180 records of asv_codes with multiple identifications. There do seem to be some aquatic taxa that have been missed (i.e. identified as an aquatic species/genus/family in one miseq run and not in another). As far as I can see, it looks like the IDs are consistent within miseq runs, but differ between them.  Does that make sense from a workflow perspective?]
#CHRIS: See my comment above. I am going to re-run the vsearch step for miseq18.
#####
...
[MEL: I would like to produce a similar set of tables for the 46-site data but am unsure where to look to find the latest, most reliable data. With a table of asv_codes and species names as they were identified in the latest version, I can easily produce a synonyms table.  I don't want to commit any of this to the mwbugs database until that is sorted, because a version of the 46-site data is in there (without the asv_code data), and that will need revising]

[MEL: note that in reformatting these tables, I have not included any of the following fields from your miseq_samples tables:
"samp_no"         "plate_position"  "i7_index"        "i5_index"        "amplicons"       "comment"
or these from your biotic table:
"Consensus"      "max_p_identity" "threshold"  "asv_seq"        "amplicon"       "reads" 
Do you think any of these are important to include in the mwbugs database (Note "kingdom","phylum","class","order","family","genus" are not included because that information is held in the taxonomic tables of the database (and implicitly in the bugcodes)),]
CHRIS: This is fine. Howvever, I'll just need to be able to create tables with the taxonomic information, max-p-identity, asv_seq, amplicon and reads, so as long as we still have this information store elsewhere so I can use it in preparing papers.

```{r}
bins <- as.data.frame(readxl::read_excel("data/asv_library_corrected.xlsx", sheet = 3))
bins <- bins[!is.na(bins$bin_uri),]

miseq_data <- bugdata_spp
