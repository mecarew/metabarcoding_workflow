...
#[MEL the 'count' field in new_biota = sum of records among the three (sometimes 2) miseq replicates. Is this sensible? Useful? Note that unidentified genera and families can then have a count > 3 if there are more than one unidentified records for a genus or family.  Would total number of reads be a more useful count value, or should we just use the value 1?]

sample_asv <- rbind(sample_asv, unique(biota_wrkng[c("asv_code","smpcode")]))
bugcode_asv <- rbind(bugcode_asv, unique(biota_wrkng[c("asv_code","bugcode")]))

#[MEL: sorry to say at the last hurdle, bugcode_asv revealed e asv_codes that have been asigned to two different species: the following returns the probelmatic 3 asv_codes. I'm pretty sure this error is not my doing. Is it easy for you to go looking for the cause?]
bugcode_asv[duplicated(bugcode_asv$asv_code) | duplicated(bugcode_asv$asv_code, fromLast = TRUE),]
