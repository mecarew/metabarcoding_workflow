...
tfill <- as.data.frame(readxl::read_excel("~/uomShare/wergStaff/MelCarew/git-data/metabarcoding_workflow/asv_library/asv_library_corrected.xlsx",
                       sheet = "tfill"))
tfill <- tfill[tfill$aqu == 1,] #19,708 records
...
tfill <- only_aqu_inverts(tfill, db, TRUE)$include # 19,706...not sure I believe that. Let's come back to it.
# remove dubious_id records
tfill <- tfill[tfill$dubious_id == 0,] #19,677
tfill_spp_bins <- as.data.frame(readxl::read_excel("~/uomShare/wergStaff/ChrisW/temp/asv_library_corrected.xlsx",
                       sheet = "bin_uris"))
tfill_spp_bins <- tfill_spp_bins[tfill_spp_bins$asv_code %in% tfill$asv_code,] #23,215 species/bin combos
bugdata <- unique(tfill[c("kingdom","phylum","class","order","family","genus","species")]) #1,160 unique identifications
bugdata_spp <- bugdata[!is.na(bugdata$species),] #1003
bugdata_gen <- bugdata[is.na(bugdata$species) & !is.na(bugdata$genus),] #94
...
new_spp <- all_spp[!all_spp %in% c(taxon_spp$species, morphospp_etc$lowesttaxon)] #578
