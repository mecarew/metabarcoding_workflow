...
load(here::here("bat_diet/cache/01.RData"))
```
...
truncation_lengths <- data.frame(
  SampleName = c("zeale"),
  forward_trunc_length = c(140),
  reverse_trunc_length = c(100)
)
...
# targets <- targets %>% filter(SampleName == "zeale")
targets <- targets %>% filter(SampleName %in% c("zeale"))
...
```{r}
paged_table(read_qza(here("bat_diet/results/dada2/zeale_denoising_stats.qza"))$data)
```
...
```{r}
zeale_table <- read_qza(here("plants/results/dada2/zeale_table.qza"))
zeale_table_df <- data.frame(zeale_table$data)
zeale_match_list <- read.table(here("plants/results/lulu/zeale_match_list.txt"))

# TODO: Currently using default parameters
z <- capture.output(zeale_lulu <- lulu(zeale_table_df, zeale_match_list))

dim(zeale_lulu$original_table)
dim(zeale_lulu$curated_table)
```

```{r}
saveRDS(zeale_lulu, file=here("plants/results/lulu/zeale_lulu.rds"))
```
...
```{r}
zeale_table <- read_qza(here("bat_diet/results/dada2/zeale_table.qza"))
zeale_table_df <- data.frame(zeale_table$data)
zeale_match_list <- read.table(here("bat_diet/results/lulu/zeale_match_list.txt"))
...
```{r}
saveRDS(zeale_lulu, file=here("bat_diet/results/lulu/zeale_lulu.rds"))
```
...
```{r}
save.image(here("bat_diet/cache/02.RData"))
```
